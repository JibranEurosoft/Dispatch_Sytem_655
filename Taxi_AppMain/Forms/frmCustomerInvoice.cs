...
namespace Taxi_AppMain
{
    public partial class frmCustomerInvoice : UI.SetupBase
    {
        ...
        private void FormatChargesGrid()
        {
            ...
            grdLister.Columns["CongtionCharge"].HeaderText = "Extra Charges";
        ...
        private void OnSave()
        {
            ...
            try
            {
                ...
                int? customerId = txtCustomerName.Tag != null ? txtCustomerName.Tag.ToIntorNull() : null;


                if (customerId == null)
                {
                    MessageBox.Show("Problem on Saving Invoice, Please select a customer again");
                    return;
        ...
        public override void DisplayRecord()
        {
            try
            {
                ...
                if (objCustomer != null)
                {
                    txtCustomerName.Tag = objMaster.Current.CustomerId;
        ...
        private void btnPickBooking_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (object[] obj in list)
                {
                    ...
                    row.Cells[COLS.Parking].Value = obj[17].ToDecimal();
                    row.Cells[COLS.Waiting].Value = obj[16].ToDecimal();
        ...
        public override void Print()
        {
            ...
            try
            {
                ...
                foreach (var item in objMaster.Current.Invoice_Charges)
                {
                    var match = list.FirstOrDefault(b => b.BookingId == item.BookingId);
                    if (match != null)
                    {
                        match.Charges = item.Booking.CustomerPrice;
                        match.ParkingCharges = (decimal)item.Booking.ParkingCharges;
                        match.WaitingCharges = (decimal)item.Booking.WaitingCharges;
                        match.ExtraDropCharges = (decimal)item.Booking.ExtraDropCharges;
                    }
                }
                frm.DataSource = list;
        ...
        private void btnCustomerLister_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    if (obj != null)
                    {
                        txtCustomerName.Tag = obj[1].ToInt();
