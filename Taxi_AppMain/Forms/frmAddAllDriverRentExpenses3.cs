...
{
    public partial class frmAddAllDriverRentExpenses3 : UI.SetupBase
    {
        ...
        public frmAddAllDriverRentExpenses3()
        {
        ...
        public void GetDrivers()
        {
            try
            {
                ...
                grdDriverRent.EnableSorting = false;
                RefreshRentView();
        ...
        private void RefreshRentView()
        {
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                 ...
                 //   var list=db.ExecuteQuery<stp_AddAllDriverRentResultEx>("exec stp_AddAllDriverRentResultEx {0},{1}").ToList().OrderBy(item => item.DriverNo, new NaturalSortComparer<string>()).ToList();
                    ...
                    for (int i = 0; i < cnt; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if (chkCarryForwardBalance.Visible == true && chkCarryForwardBalance.Checked == false)
                            {
                                ...
                                grdDriverRent.Rows[i].Cells[COLS.OldBalance].Value = 0.00m;
                            }
                        }
                        ...
                        grdDriverRent.Rows[i].Cells[COLS.VAT].Value = list[i].VAT.ToDecimal();

                               

                       // grdDriverRent.Rows[i].Cells[COLS.BookingFee].Value = list[i].BookingFee.ToDecimal();
                      //  grdDriverRent.Rows[i].Cells[COLS.AccBookingFee].Value = list[i].AccBookingFee.ToDecimal();
        ...
        private bool OnSave(int DriverId, decimal oldBalance, decimal DriverRent, decimal pdaRent, decimal InitialBalance, decimal rentPayValue, ref decimal accJobs, ref long transId, ref decimal rentDue, ref decimal currBalance, decimal CarRent, decimal CarInsuranceRent, decimal PrimeCompanyRent, double TotalWeeks, bool IsHoliday, decimal VAT)
        {
            ...
            try
            {
                ...
                DateTime fromDate = dtpFromDate.Value.Value.Date;
                DateTime tillDate = dtpTillDate.Value.Value.Date;
                ...
                var rentList = db.DriverRent_Charges.Where(c => c.BookingId != null );
                ...
                List<DriverRent_Charge> ListDetail = null;
                decimal Total = 0.00m;


                decimal CashJobsTotal = 0.00m;


                decimal ACCJobsTotal = 0.00m;

                decimal parkingTotal = 0.00m;

                decimal extraTotal = 0.00m;


                decimal AgentCommission = 0.00m;
                ...
                {
                    var list2 = (from a in db.Bookings.Where(c => (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED || (c.PaymentTypeId!=Enums.PAYMENT_TYPES.CASH && c.BookingStatusId==Enums.BOOKINGSTATUS.NOPICKUP))
                                 ...
                                 {
                                     Id = a.Id,
                                     TotalFare = a.FareRate,
                                     a.MeetAndGreetCharges,
                    ...
                    Total = list2.Sum(c => (c.TotalFare + c.MeetAndGreetCharges.ToDecimal())).ToDecimal();


                    CashJobsTotal = list2.Where(c => c.PaymentTypeId.ToInt() == Enums.PAYMENT_TYPES.CASH)
                                          .Sum(c => (c.TotalFare + c.MeetAndGreetCharges.ToDecimal()) ).ToDecimal();


                    ACCJobsTotal = list2.Where(c => c.PaymentTypeId.ToInt() != Enums.PAYMENT_TYPES.CASH)
                                          .Sum(c => (c.TotalFare + c.MeetAndGreetCharges.ToDecimal())).ToDecimal();

                    parkingTotal = list2.Where(c => c.PaymentTypeId != Enums.PAYMENT_TYPES.CASH)
                                                                 .Sum(c => c.Parking.ToDecimal());

                    extraTotal = list2.Where(c => c.PaymentTypeId != Enums.PAYMENT_TYPES.CASH)
                                                               .Sum(c => c.Extra.ToDecimal());


                    AgentCommission = list2.Where(c => c.PaymentTypeId == Enums.PAYMENT_TYPES.CASH).Sum(c => c.AgentCommission.ToDecimal()).ToDecimal();
                    AgentCommission = AgentCommission + list2.Where(c => c.PaymentTypeId == Enums.PAYMENT_TYPES.CASH).Sum(c => c.BookingFee.ToDecimal()).ToDecimal();

                    accBookingFee =  list2.Where(c => c.PaymentTypeId != Enums.PAYMENT_TYPES.CASH).Sum(c => c.BookingFee.ToDecimal()).ToDecimal();


                    jobsCount = list2.Count;

                }
              else  if (AppVars.objPolicyConfiguration.PickBookingOnInvoicingType.ToInt() == 4)
                {
                    var list2 = (from a in db.Bookings.Where(c => (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED 
                                 || (c.PaymentTypeId != Enums.PAYMENT_TYPES.CASH && ( c.BookingStatusId == Enums.BOOKINGSTATUS.NOPICKUP)))
                ...
                //decimal extraTotal = list2.Where(c => c.AccountTypeId != Enums.ACCOUNT_TYPE.CASH)
                //                                            .Sum(c => c.Extra.ToDecimal());
                ...
                objMaster.Current.OldAgentCommission = accBookingFee.ToDecimal();
                ...
                totalCredit = (ACCJobsTotal + Credit + parkingTotal+ extraTotal);
                ...
                objMaster.Current.TotalJobs = jobsCount;

                objMaster.Current.Extra = extraTotal;
        ...
        private void btnViewPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    frmDriverRentTransactionExpensesReport3 frm = new frmDriverRentTransactionExpensesReport3(list, dtpFromDate.Value.ToDate(), dtpTillDate.Value.ToDate());
