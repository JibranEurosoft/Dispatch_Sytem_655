...
namespace Taxi_AppMain
{
    public partial class frmOnlineBookingSettings : UI.SetupBase
    {
        ...
        public frmOnlineBookingSettings()
        {
            ...
            grdZones.CellEndEdit += GrdZones_CellEndEdit;
        }

        private void GrdZones_CellEndEdit(object sender, GridViewCellEventArgs e)
        {
            if (e.Column.Name == COL_ZONE.BlockPickup || e.Column.Name == COL_ZONE.BlockDropOff || e.Column.Name == COL_ZONE.IsLocalArea)
            {

                e.Row.Cells[COL_ZONE.IsUpdated].Value = "1";
            }
        }
        ...
        public struct COL_ZONE
        {
            ...
            public static string BlockPickupOld = "BlockPickupOld";
            public static string BlockDropOffOld = "BlockDropOffOld";
            public static string IsLocalAreaOld = "IsLocalAreaOld";
            ...
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    for (int i = 0; i < data1.Count; i++)
                    {
                        ...
                        grdZones.Rows[i].Cells[COL_ZONE.BlockPickupOld].Value = data1[i].BlockPickup;
                        grdZones.Rows[i].Cells[COL_ZONE.BlockDropOffOld].Value = data1[i].BlockDropOff;
                        grdZones.Rows[i].Cells[COL_ZONE.IsLocalAreaOld].Value = data1[i].IsOutsideArea;
                    ...
                    if  (AppVars.listUserRights.Count(c => c.functionId == "SHOW AUTHORIZATION BOX") > 0)
                    {
                        try
                        {
                            chkEnableAuth.Visible = true;
                            chkEnableAuth.Checked = db.Gen_SysPolicy_Configurations
                                .Select(c => c.EnableMobileBookingAuthorization).FirstOrDefault().ToBool();
//
                        }
                        catch
                        {
        ...
        private void FormateGrid()
        {
            try
            {
                ...
                cbcol = new GridViewCheckBoxColumn();
                cbcol.Name = COL_ZONE.IsLocalAreaOld;
                cbcol.HeaderText = "Outside Area";
                cbcol.Width = 100;
                cbcol.ReadOnly = false;
                ...
                cbcol = new GridViewCheckBoxColumn();
                cbcol.Name = COL_ZONE.BlockPickupOld;
                cbcol.HeaderText = "Block Pickup";
                cbcol.Width = 100;
                cbcol.ReadOnly = false;
                cbcol.IsVisible = false;
                ...
                cbcol = new GridViewCheckBoxColumn();
                cbcol.Name = COL_ZONE.BlockDropOffOld;
                cbcol.HeaderText = "Out Of Town";
                cbcol.Width = 100;
                cbcol.IsVisible = false;
                cbcol.ReadOnly = false;
        ...
        private void UpdateZones()
        {
            ...
            try
            {
                ...
                {
                    if (grdZones.Rows[i].Cells[COL_ZONE.IsUpdated].Value.ToStr() == "1")
                    {
            ...
            try
            {
                ...
                if (chkEnableAuth.Visible)
                {
                    ...
                    try
                    {
                        using (TaxiDataContext db = new TaxiDataContext())
                        {
                            if (chkEnableAuth.Checked)
                            {
                                db.ExecuteQuery<int>("update gen_syspolicy_configurations set EnableMobileBookingAuthorization=1,EnableOnlineBookingAuthorization=1");

                            }
                            else
                            {
                                db.ExecuteQuery<int>("update gen_syspolicy_configurations set EnableMobileBookingAuthorization=0,EnableOnlineBookingAuthorization=0");
