      ...
      private void DisposeTimer()
    {
        ...
        private void TrackDriver()
        {
            try
            {
                ...
                isfullmap = ((System.Configuration.ConfigurationManager.AppSettings["trackdriveronmapapp"] != null
                    && System.Configuration.ConfigurationManager.AppSettings["trackdriveronmapapp"].ToStr() == "1")

                    || AppVars.listUserRights.Count(c => c.functionId == "TRACK DRIVER ON MAIN MAP") > 0);

                if(isfullmap)
                {
                    if( Process.GetProcesses().Where(c => c.ProcessName.ToLower() == "mapapp").Count()     ==0)
                    {
                        isfullmap = false;
