...
namespace Taxi_AppMain
{
    public partial class frmAdvanceBookingCustomization : RadForm
    {
        ...
        int? ReturnEscortId = 0;
        int? ReturnDepartmentId = 0;
        int? ReturnSubCompanyId = 0;
        int? ReturnPaymentTypeId = 0;
        int? ReturnVehicleTypeId = 0;       
        int? ReturnPickupPlot = 0;
        int? ReturnDropOffPlot = 0;
        ...
        void ddlReturnToLocType_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
        ...
        private void InitializeFormSettings()
        {
            try
            {
             ...
             //   ddlToLocType.SelectedIndexChanged += new EventHandler(ddlToLocType_SelectedIndexChanged);
                ...
                ddlFromLocType.SelectedIndexChanged += new Telerik.WinControls.UI.Data.PositionChangedEventHandler(ddlFromLocType_SelectedIndexChanged);

                ddlToLocType.SelectedIndexChanged += new Telerik.WinControls.UI.Data.PositionChangedEventHandler(ddlToLocType_SelectedIndexChanged);
                ...
                ddlReturnFromLocType.SelectedIndexChanged += new Telerik.WinControls.UI.Data.PositionChangedEventHandler(ddlReturnFromLocType_SelectedIndexChanged);

                ddlReturnToLocType.SelectedIndexChanged += new Telerik.WinControls.UI.Data.PositionChangedEventHandler(ddlReturnToLocType_SelectedIndexChanged);
                ...
                {
                    foreach (var item in radPanel5.Controls)
                    {
                        if (item is RadTextBox)
                        {
                            (item as RadTextBox).BackColor = Color.Black;
                            (item as RadTextBox).ForeColor = Color.White;
                        }
                        if (item is RadLabel)
                        {
                            (item as RadLabel).BackColor = Color.Black;
                            (item as RadLabel).ForeColor = Color.White;
                        }
                        if (item is RadCheckBox)
                        {
                            (item as RadCheckBox).BackColor = Color.Black;
                            (item as RadCheckBox).ForeColor = Color.White;
                        }
                    ...
                    foreach (var item in radPanel7.Controls)
                    {
                        if (item is RadTextBox)
                        {
                            (item as RadTextBox).BackColor = Color.Black;
                    ...
                    foreach (var item in gbCancelBooking.Controls)
                    {
                        if (item is RadTextBox)
                        {
                            (item as RadTextBox).BackColor = Color.Black;
                    ...
                    chkShowOneWayDispatchedBookings.BackColor = Color.Black;
                    chkShowOneWayDispatchedBookings.ForeColor = Color.White;


                    chkShowReturnDispatchedBookings.BackColor = Color.Black;
                    chkShowReturnDispatchedBookings.ForeColor = Color.White;

                    grdBookings.ForeColor = Color.White;
                    grdReturnBookings.ForeColor = Color.White;
                    chkCancelBooking.BackColor = Color.Black;
                    chkCancelBooking.ForeColor = Color.White;

                    chkReturnCancelBooking.BackColor = Color.Black;
                    chkReturnCancelBooking.ForeColor = Color.White;


                    radLabel3.BackColor = Color.Black;
                    ...
                    radLabel47.BackColor = Color.Black;
                    radLabel47.ForeColor = Color.White;
                }
            }
            catch (Exception ex)
            {
        ...
        private void FillCombo()
        {
            ...
            ComboFunctions.FillEscortCombo(ddlescortOneway,null);
            ...
            using (TaxiDataContext db = new TaxiDataContext())
            {
                    ...
                    var dt = (from a in db.Gen_Zones.Where(c => c.ZoneName != "SIN BIN")
                                orderby a.OrderNo
                                select new
                                {
                                    Id = a.Id,
            ...
            FillPlotCombo(ddlPickupPlot, dt);
            FillPlotCombo(ddlDropOffPlot, dt.ToList());
            }
        ...
        private void FillReturnCombo()
        {
            ...
            ComboFunctions.FillSubCompanyCombo(ddlSubCompanyReturn);
            ComboFunctions.FillEscortCombo(ddlEscortReturn,null);
            ...
            using (TaxiDataContext db = new TaxiDataContext())
            {
                ...
                FillPlotCombo(ddlPickupPlotReturn, dt);
                FillPlotCombo(ddlDropOffPlotReturn, dt.ToList());
            }
            IsPaymentTypeLoaded = true;
        ...
        private void DisplayOneWayBooking()
        {
            try
            {
                ...
                grdPickupDates.BeginUpdate();
                 ...
                 objFirstBooking = oneWayBookingList.FirstOrDefault(c=>c.BookingStatusId == Enums.BOOKINGSTATUS.WAITING);
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    if (i == 0)
                    {
                        ...
                        numLead.Value = objFirstBooking.DeadMileage.ToDecimal();

                        if (numLead.Value > 0)
                            chkLead.Checked = true;

                        if (objFirstBooking.ZoneId.ToInt() > 0)
                            ddlPickupPlot.SelectedValue = objFirstBooking.ZoneId.ToInt();

                        if (objFirstBooking.DropOffZoneId.ToInt() > 0)
                            ddlDropOffPlot.SelectedValue = objFirstBooking.DropOffZoneId.ToInt();

                     

                        ddlSubCompany.SelectedValue = objFirstBooking.SubcompanyId.ToInt();
                        ...
                        numEscort.Value = objFirstBooking.EscortPrice.ToDecimal();
                        chkIsQuoted.Checked = objFirstBooking.IsQuotedPrice.ToBool();
                        ...
                        if (objFirstBooking.DepartmentId.ToInt() > 0)
                            ddlDepartment.SelectedValue = objFirstBooking.DepartmentId;

                        ddlescortOneway.SelectedValue = objFirstBooking.EscortId;
                        ...
                        if (oneWayBookingList.Count(c => c.BookingStatusId == 26) > 0)
                        {
                            chkSuspended.Text = "Resume";
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            numLeadReturn.Value = objFirstBooking.DeadMileage.ToDecimal();

                            if (numLeadReturn.Value > 0)
                                chkLeadReturn.Checked = true;

                            if (objFirstBooking.ZoneId.ToInt() > 0)
                                ddlPickupPlotReturn.SelectedValue = objFirstBooking.ZoneId.ToInt();

                            if (objFirstBooking.DropOffZoneId.ToInt() > 0)
                                ddlDropOffPlotReturn.SelectedValue = objFirstBooking.DropOffZoneId.ToInt();

                            
                            ddlSubCompanyReturn.SelectedValue = objFirstBooking.SubcompanyId.ToInt();
                            ...
                            numReturnCustomerPrice.Value = objFirstBooking.FareRate.ToDecimal();
                            ...
                            chkIsReturnQuoted.Checked = objFirstBooking.IsQuotedPrice.ToBool();
                            ...
                            if (objFirstBooking.DepartmentId.ToInt() > 0)
                                ddlDepartmentReturn.SelectedValue = objFirstBooking.DepartmentId;

                            ddlEscortReturn.SelectedValue = objFirstBooking.EscortId;
        ...
        void frmMultiBooking_Shown(object sender, EventArgs e)
        {
            ...
            if (AppVars.listUserRights.Count(c => c.functionId == "HIDE ACCOUNT FARES") > 0 || AppVars.listUserRights.Count(c => c.functionId == "HIDE COMPANY PRICE") > 0)
            {
                if (numCompanyFares != null)
                {
                    numCompanyFares.Visible = false;

                    chkCompanyPrice.Visible = false;

                    chkReturnCompanyPrice.Visible = false;
                    numReturnCompanyPrice.Visible = false;
                }
            }
                    ...
                    if (IsSuccess)
                    {
                        ...
                        try
                        {
                            ...
                            if (chkPickup.Checked || chkDestination.Checked)
                            {
                                ...
                                if (objAdv != null)
                                {
                                    ...
                                    objAdv.OrderNo = txtOrderNo.Text.Trim();
                                    db.SubmitChanges();
                            ...
                            else if (chkSuspended.Checked==false)
                            {
                                if(ObjAdvanceBooking.OrderNo.ToStr().Trim()!=txtOrderNo.Text.Trim())
                                {
                                    AdvanceBooking objAdv = db.AdvanceBookings.FirstOrDefault(c => c.Id == ObjAdvanceBooking.Id);
                        ...
                        if (chkSuspended.Checked)
                        {
                            try
                            {
                                ...
                                int advancebookingtypeid = 2;
                                if (chkSuspended.Text.ToLower().StartsWith("suspend"))
                                {
                                    advancebookingtypeid = 3;
                                ...
                                if (objAdv != null)
                                {
                                    ...
                                    objAdv.AdvBookingTypeId = advancebookingtypeid;

                                    db.SubmitChanges();



                                    //    General.RefreshListWithoutSelectedOnRefreshData<frmAdvanceBookingsList>("frmAdvanceBookingsList1");
                                }
                                //    }
                            }
                            catch
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    if (grdBookings.Rows[i].Cells[COLS.EXCLUDED].Value.ToStr() == "1")
                    {
                        ...
                        */
                        if (objBooking == null)
                        {
                            ...
                            {
                                db.stp_CancelBookingWithUserLog(bookingId, "Excluded from Multi Booking", AppVars.LoginObj.UserName.ToStr(), AppVars.LoginObj.LuserId.ToIntorNull());
                    ...
                    else
                    {
                        ...
                        else if (long.TryParse(grdBookings.Rows[i].Cells[COLS.ID].Value.ToStr(), out bookingId))
                        {
                            ...
                            objBooking.IsQuotedPrice =isquoted;
                            ...
                            objBooking.SubcompanyId = SubCompanyId;

                            objBooking.ZoneId = PickupPlot;
                            objBooking.DropOffZoneId = DropOffPlot;

                            if (ddlDepartment != null)
                                objBooking.DepartmentId = DepartmentId;
                            else
                                objBooking.DepartmentId = null;
                            ...
                            if (objBooking.BookingStatusId == Enums.BOOKINGSTATUS.WAITING && chkSuspended.Checked)
                            {
                                if (chkSuspended.Checked && chkSuspended.Text == "Resume")
                                {

                                    if (objBooking.PickupDateTime.ToDate() >= DateTime.Now.ToDate())
                                    {
                            ...
                            else
                            {
                                if (chkSuspended.Checked && chkSuspended.Text == "Resume")
                                {
                                    if (objBooking.BookingStatusId.ToInt() == 26)
                                    {
                                        ...
                                        {
                                            objBooking.BookingStatusId = Enums.BOOKINGSTATUS.CANCELLED;
                            ...
                            if (chkLead.Checked)
                            {
                                objBooking.AutoDespatchTime = grdBookings.Rows[i].Cells[COLS.PickupDate].Value.ToDateTimeorNull().Value.AddMinutes(-numLead.Value.ToInt()).ToDateTime();
                                objBooking.DeadMileage = numLead.Value;
                            }
                            else
                            {
                                objBooking.AutoDespatchTime = null;
                                objBooking.DeadMileage = 0;
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    else
                    {
                        ...
                        else if (long.TryParse(grdReturnBookings.Rows[i].Cells[COLS.ID].Value.ToStr(), out bookingId))
                        {
                            ...
                            objBooking.IsQuotedPrice = ReturnQuoted;
                            ...
                            objBooking.SubcompanyId = ReturnSubCompanyId;

                            objBooking.ZoneId = ReturnPickupPlot;
                            objBooking.DropOffZoneId = ReturnDropOffPlot;

                            if (ReturnEscortId == 0)
                                ReturnEscortId = null;
                            objBooking.EscortId = ReturnEscortId;
                            ...
                            if (ddlDepartmentReturn != null)
                                objBooking.DepartmentId = ReturnDepartmentId;
                            else
                                objBooking.DepartmentId = null;
                            ...
                            if (chkLeadReturn.Checked)
                            {
                                objBooking.AutoDespatchTime = grdReturnBookings.Rows[i].Cells[COLS.PickupDate].Value.ToDateTimeorNull().Value.AddMinutes(-numLeadReturn.Value.ToInt()).ToDateTime();
                                objBooking.DeadMileage = numLeadReturn.Value;
                            }
                            else
                            {
                                objBooking.AutoDespatchTime = null;
                            ...
                            if (ReturncPickupFares)
                            {
                                ...
                                objBooking.CustomerPrice = objBooking.FareRate;
                            }
        ...
        private bool SaveChanges()
        {
            ...
            EscortId = ddlescortOneway.SelectedValue.ToIntorNull();
            PaymentTypeId = ddlPaymentType.SelectedValue.ToInt();
            VehicleTypeId = ddlVehicle.SelectedValue.ToIntorNull();
            PickupPlot = ddlPickupPlot.SelectedValue.ToIntorNull();
            DropOffPlot = ddlDropOffPlot.SelectedValue.ToIntorNull();
            DepartmentId = ddlDepartment.SelectedValue.ToIntorNull();
            SubCompanyId = ddlSubCompany.SelectedValue.ToIntorNull();
            ...
            ReturnQuoted = chkIsReturnQuoted.Checked;
            ...
            ReturnEscortId = ddlEscortReturn.SelectedValue.ToIntorNull();
            ReturnPickupPlot = ddlPickupPlotReturn.SelectedValue.ToIntorNull();
            ReturnDropOffPlot = ddlDropOffPlotReturn.SelectedValue.ToIntorNull();
            ...
            ReturnDepartmentId = ddlDepartmentReturn.SelectedValue.ToIntorNull();
            ReturnSubCompanyId = ddlSubCompanyReturn.SelectedValue.ToIntorNull();
        ...
        private void ApplyChanges()
        {
            try
            {
                ...
                //{
                    ...
                    else if (chkStartingAt.Checked && chkEndingAt.Checked)
                    {
        ...
        private void ApplyReturnChanges()
        {
            try
            {
                ...
                if (chkReturnDayWise.Checked.ToBool())
                {
                    string DayName = string.Empty;
                    ...
                    foreach (var gRow in grdReturnBookings.Rows.Where(c => c.Cells[COLS.EXCLUDED].Value.ToStr() == ""))
                    {
                        if (gRow.Cells[COLS.DAY].Value.ToStr().StartsWith(DayName))
                        {
                            ...
                            if (chkReturnPickupTime.Checked &&  dtpReturnPickupTime.Value != null && gRow.Cells[COLS.PickupDate].Value != null)
                            {
                ...
                else if (chkReturnStartingAt.Checked && chkReturnEndingAt.Checked)
                {
        ...
        //private void ddlFromLocType_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventHandler e)
        //{
        //    FillFromLocations();
        ...
        void ddlReturnFromLocType_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
        ...
        private void CalculateTotalFares()
        {
            ...
            try
            {
                ...
                string fromLocName = ddlFromLocation.SelectedItem != null ? (ddlFromLocation.SelectedItem as RadListDataItem).Text.Trim() : ddlFromLocation.Text.Trim();
                string toLocName = ddlToLocation.SelectedItem != null ? (ddlToLocation.SelectedItem as RadListDataItem).Text.Trim() : ddlToLocation.Text.Trim();
                ...
                if (grdBookings != null && grdBookings.Rows.Count > 0)
                {
                    ...
                    if (query.Length > 0)
                    {
                        using (TaxiDataContext db = new TaxiDataContext())
                        {
                            ...
                            // bool isAllCancel = cancelcnt == grdBookings.Rows.Count;

                            // if(isa)
                          //  int isAllCancel = db.Bookings.Where(c => c.AdvanceBookingId == ObjAdvanceBooking.Id && c.BookingStatusId == Enums.BOOKINGSTATUS.CANCELLED).Count();


                           // int totalBookings = db.Bookings.Where(c => c.AdvanceBookingId == ObjAdvanceBooking.Id).Count();



                            if (grdReturnBookings.Rows.Count==0)
                            {

                                db.ExecuteQuery<int>("update advancebooking set AdvBookingTypeId=4 where id=" + ObjAdvanceBooking.Id);
        ...
        private void chkEscortPrice_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                numEscort.Enabled = true;
            }
            else
            {
                numEscort.Enabled = false;
            }
        }
        ...
        private void ddlAccount_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    FillDepartmentsCombo(AccountId);
                }
            }
            catch (Exception ex)
            {
        ...
        private void ddlReturnAccount_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            try
            {
                ...
                if (accountTypeId == Enums.ACCOUNT_TYPE.CASH)
                {
                    ...
                    ClearDepartmentReturn();
                }
                else
                {
                    ddlReturnPaymentType.SelectedValue = Enums.PAYMENT_TYPES.BANK_ACCOUNT;
                    ddlDepartmentReturn.Enabled = true;
                    FillDepartmentsReturnCombo(AccountId);
        ...
        private void chkShowReturnDispatchedBookings_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                ...
                grdReturnBookings.Rows.Where(c => c.Cells[COLS.StatusId].Value.ToInt() != Enums.BOOKINGSTATUS.WAITING
                                          ).ToList().ForEach(c => c.IsVisible = true);
            }
            else
            {
               ...
               && aTxt.Text.Length > 0)
            {
                ...
                else
                {
                    ...
                    if (aTxt.Name == "txtReturnFromAddress" || aTxt.Name == "txtReturnToAddress")
                    {
                        ...
                        {
                            if (aTxt.Name == "txtReturnFromAddress")
                                ddlPickupPlotReturn.SelectedValue = General.GetZoneId(aTxt.SelectedItem.ToStr().ToUpper());
                            else if (aTxt.Name == "txtReturnToAddress")
                                ddlDropOffPlotReturn.SelectedValue = General.GetZoneId(aTxt.SelectedItem.ToStr().ToUpper());
                        }
                        catch
                        {
                    ...
                    else
                    {
                        ...
                        {
                            if (aTxt.Name == "txtFromAddress")
                                ddlPickupPlot.SelectedValue = General.GetZoneId(aTxt.SelectedItem.ToStr().ToUpper());
                            else if (aTxt.Name == "txtToAddress")
                                ddlDropOffPlot.SelectedValue = General.GetZoneId(aTxt.SelectedItem.ToStr().ToUpper());
        ...
        private void ClearDepartmentReturn()
        {
            if (ddlDepartmentReturn != null)
                ddlDepartmentReturn.DataSource = null;
        }

        private void FillDepartmentsReturnCombo(int companyId)
        {
            ComboFunctions.FillCompanyDepartmentCombo(ddlDepartmentReturn, c => c.CompanyId == companyId);
        }
        ...
        protected void WithInSave(Booking Current)
        {
            ...
            Current.AutoDespatchTime = Current.AcceptedDateTime;
            Current.DeadMileage = Current.DeadMileage;

            Current.DepartmentId = Current.DepartmentId;

            Current.ZoneId = Current.ZoneId;
            Current.DropOffZoneId = Current.DropOffZoneId;
            ...
            if (Current.SubcompanyId == null)
            {
                ...
                if (subCompanyId > 0)
                {
                    Current.SubcompanyId = Current.SubcompanyId;
        ...
        private void chkLeadReturn_CheckedChanged(object sender, EventArgs e)
        {
            if (chkLeadReturn.Checked)
                numLeadReturn.Enabled = true;
            else
                numLeadReturn.Enabled = false;
        }
        ...
        private void btnSaveExtendBooking_Click(object sender, EventArgs e)
        {
            if(ObjAdvanceBooking.AdvBookingTypeId.ToInt()==3)
            {
                MessageBox.Show("Cannot extend suspended multibooking");
                return;

            }


            var days= grdPickupDates.Rows.Select(c => c.Cells[COLS_PICKUPS.DAY].Value.ToStr()).Distinct().ToList();
            frmMultiBooking frm = new frmMultiBooking(objFirstBooking, ObjAdvanceBooking.Id, days,dtpStartingAt.Value.ToDate(),dtpEndingAt.Value.ToDate());
            frm.ShowDialog();
            frm.Dispose();
            DisplayBookings();
        }
