...
namespace Taxi_AppMain
{
    public partial class frmDrivertemplet :UI.SetupBase
    {
        ...
        private void FormatSMSTempletGrid()
        {
            ...
            GridFunctions.SetFilter(grdSMSTemplets);
        }
        ...
        private void Display()
        {
            try
            {
                if (objMaster.Current != null)
                {
                    ...
                    var list = objMaster.Current.Fleet_DriverTemplets.OrderByDescending(c => c.Id).Take(2000).Select(args => new { args.Id, args.SysPolicyId, args.Templets }).ToList();

                    grdSMSTemplets.RowCount = list.Count;
                    

                    for (int i=0; i < list.Count; i++)
                    {
                        //  objRowTemplet = grdSMSTemplets.Rows.AddNew();
                        grdSMSTemplets.Rows[i].Cells[COL_SMSTEMPLET.ID].Value = list[i].Id;
                        grdSMSTemplets.Rows[i].Cells[COL_SMSTEMPLET.POLICYID].Value = list[i].SysPolicyId;
                        grdSMSTemplets.Rows[i].Cells[COL_SMSTEMPLET.Tempplet].Value = list[i].Templets;
