...
{
    public partial class frmSentSMS : Forms.BaseForm
    {
        ...
        void grdLister_ViewCellFormatting(object sender, CellFormattingEventArgs e)
        {
            if (e.CellElement is GridDataCellElement)
            {
                ...
                e.CellElement.ToolTipText = e.CellElement.RowInfo.Cells["Message"].Value.ToStr();
        ...
        private void PopulateData()
        {
            ...
            try
            {
                DateTime filter = DateTime.Now.AddDays(-90).ToDate();
                using (Taxi_Model.TaxiDataContext db = new Taxi_Model.TaxiDataContext())
                {

                    grdLister.DataSource =db.SentSMs
                        .Where(c => c.SentOn >= filter)
                        .Select(args => new
                        {
                            Id = args.Id,
                            To = args.SentTo,
                            Message = args.SMSBody,
                            SentOn = args.SentOn,
                            By = args.SentBy
                    ...
                    grdLister.Columns["Message"].Width = 500;
