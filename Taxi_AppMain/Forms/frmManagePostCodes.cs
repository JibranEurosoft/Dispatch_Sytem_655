    ...
    public partial class frmManagePostCodes : UI.SetupBase
    {
        ...
        void UpdateCurrent_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DialogResult.Yes == MessageBox.Show("Do you  want to update coordinates ? " + Environment.NewLine + "This will take a few minutes to update", "Update Coordinates", MessageBoxButtons.YesNo))
                {
                    ...
                    UpdateCurrentworker.RunWorkerAsync(grdSelectedPostCodes.CurrentRow);
        ...
        void UpdateCurrentworker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                GridViewRowInfo rowX = null;

                if (e.Argument is List<GridViewRowInfo>)
                    rows = (List<GridViewRowInfo>)e.Argument;
                else
                {
                    rowX = (GridViewRowInfo)e.Argument;

                    rows = new List<GridViewRowInfo>();
                    rows.Add(rowX);
                }

                if (rowX != null && rowX.Cells[COLS.Id].Value.ToInt() == 0)
                {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    if (allowUpdating)
                    {
                        ...
                        int sbCount = 0;
                        int totalCoordinates = listofCoordinates.Count;

                        int twentyPercent = (totalCoordinates * 1) / 100;
                        ...
                        foreach (var item2 in listofCoordinates)
                        {
                            try
                            {
                                ...
                                sb.AppendLine("INSERT INTO GEN_COORDINATES(PostCode,Latitude,Longitude)VALUES('" + item2.PostCode + "'," + item2.Latitude + "," + item2.Longitude + ")");


                                sbDelete.AppendLine("DELETE FROM GEN_COORDINATES WHERE POSTCODE='" + item2.PostCode + "';");


                                sbCount++;

                                if (sbCount == twentyPercent || (totalIteration > 99 && sbCount > 0 && sbCount == (totalCoordinates - totalUpdate)))
                                {
                                    ...
                                    catch
                                    {
                                        try
                                        {
                                            db.ExecuteQuery<int>(sbDelete.ToStr());
                                    ...
                                    totalUpdate += sbCount;
                                    cr.index = totalUpdate;
                                    UpdateCurrentworker.ReportProgress(totalUpdate, cr);
                                    sbCount = 0;
                                    sb.Clear();
                                    sbDelete.Clear();
                                    totalIteration++;
            ...
            try
            {
                ...
                if (DialogResult.Yes == MessageBox.Show("Do you  want to update coordinates ? " + Environment.NewLine + "This will take a few minutes to update", "Update Coordinates", MessageBoxButtons.YesNo))
                {
                    ...
                    lblUpdate.Text = string.Empty;
                        ...
                        UpdateCurrentworker.RunWorkerAsync(grdSelectedPostCodes.Rows.ToList());
