    ...
    public partial class frmInActiveDriversList : UI.SetupBase
    {
        ...
        void frmDriversList_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                grdLister.Columns["EndDate"].IsVisible = true;

                (grdLister.Columns["EndDate"] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy }";
                (grdLister.Columns["EndDate"] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy";
        ...
        public override void PopulateData()
        {
            try
            {
                ...
                {
                    var avail = db.ExecuteQuery<clsAvail>("SELECT DriverId,EndingDate=endingdate FROM Fleet_Driver_Availability where id in(select Max(Id) from  Fleet_Driver_Availability group by driverid)").ToList();
                    ...
                    if (FilterType == 1)
                    {
                        ...
                        var query = (from a in General.GetQueryable<Fleet_Driver>(null).AsEnumerable()
                                     where a.IsActive == false || avail.Count(c=>c.DriverId==a.Id && c.EndingDate!=null)>0
                                     ...
                                     select new
                                     {
                                         ...
                                         MobileNo = a.MobileNo,
                                         EndDate = avail.FirstOrDefault(c=>c.DriverId==a.Id).DefaultIfEmpty().EndingDate
                    ...
                    else if (FilterType == 2)
                    {
                        ...
                        var query = (from a in General.GetQueryable<Fleet_Driver>(b => b.IsActive == false).AsEnumerable()
                                     where avail.Count(c => c.DriverId == a.Id && c.EndingDate==null) >0
