...
namespace Taxi_AppMain
{
    public partial class frmInvoiceCreditNotes : UI.SetupBase
    {
            ...
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                 ...
                 //   objInvoiceCreditNote.InvoiceCreditNotes_Charges
                    ...
                    objInvoiceCreditNote.CreditNoteDate = dtpCreditDate.Value.ToDate();
                    objInvoiceCreditNote.CreditNoteTotal = grdLister.Rows.Where(c => c.Cells[COLS.PaymentTypeId].Value.ToInt() != 6 && c.Cells["Check"].Value.ToBool() == true).Sum(c => c.Cells[COLS.CreditAmount].Value.ToDecimal());
        ...
        public override void DisplayRecord()
        {
            ...
            bool showZeroChargesInDefault = false;
            ...
            using (TaxiDataContext db = new TaxiDataContext())
            {
                ...
                if(AppVars.listUserRights.Count(c=>c.functionId== "SHOW ZERO CHARGES DEFAULT ON CREDIT NOTE") >0)
                {
                    showZeroChargesInDefault = true;
            ...
            grdLister.BeginUpdate();
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    if(showZeroChargesInDefault)
                    {
                       // grdLister.Rows[i].Cells[COLS.Charges].Value = 0.00m;
                        grdLister.Rows[i].Cells[COLS.CreditAmount].Value = 0.00m;
                        //grdLister.Rows[i].Cells[COLS.Parking].Value = 0.00m;
                       // grdLister.Rows[i].Cells[COLS.Waiting].Value = 0.00m;
                       // grdLister.Rows[i].Cells[COLS.ExtraDrop].Value = 0.00m;
                        //grdLister.Rows[i].Cells[COLS.BOOKINGFEE].Value = 0.00m;
                ...
                //    grdLister.Rows[i].Cells["Check"].Value = true;
                    ...
                    //if (TemplateName == "Template13" || TemplateName == "chec"Template24")
                    //{
            ...
            if (templateName == "Template13" || TemplateName == "Template24")
            {
                ...
                var list = (from b in list1
                            ...
                            select new
                            {
                                ...
                                Tip=b.ServiceCharges.ToDecimal(),
                            }).ToList();
        ...
        private void CalculateTotal()
        {
            ...
            txtCreditAmount.Text = grdLister.Rows.Where(c=>c.Cells[COLS.PaymentTypeId].Value.ToInt()!=6 && c.Cells["Check"].Value.ToBool() == true)
                                                    .Sum(c => c.Cells[COLS.CreditAmount].Value.ToDecimal()).ToString();
