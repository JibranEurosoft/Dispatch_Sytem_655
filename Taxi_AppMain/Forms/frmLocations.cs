       ...
       // bool IsLocationTypeLoaded = false;
        ...
        void frmLocations_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string[] attributeValuesArr = objMaster.Current.City.ToStr().Trim().Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                if (objMaster.Current== null || objMaster.Current.LocationTypeId.ToInt() != Enums.LOCATION_TYPES.AIRPORT)
                {
                    if ( AppVars.listUserRights.Count(c => c.functionId == "SHOW LOCATION ATTRIBUTES") > 0 )
                    {
                        label1.Visible = true;
                        grdDriverAttributes.Visible = true;
                    }
                ...
                else
                {
                    ...
                    grdDriverAttributes.Visible = false;
                }
            }
            catch
            {
        ...
        public override void Save()
        {
            try
            {
                ...
                if (grdDriverAttributes != null)
                {
                    ...
                    try
                    {
                        objMaster.Current.City = string.Join(",", grdDriverAttributes.Rows.Where(c => c.Cells[Col_DriverAttributes.Default].Value.ToBool() == true).Select(c => c.Cells[Col_DriverAttributes.ShortName].Value.ToStr()).ToArray<string>());

                        if (objMaster.Current.City.ToStr().Trim().Length > 0)
                        {

                            objMaster.Current.City = "," + objMaster.Current.City.ToStr() + ",";
                        }
