...
namespace Taxi_AppMain
{
    ...
    public partial class frmMultiBooking : RadForm
    {
        ...
        public decimal ReturnExtraDrop = 0.00m;
        ...
        public frmMultiBooking(Booking objMasterBooking,long advanceBookingId=0,List<string> days=null,DateTime? startDate=null,DateTime? endDate=null)
        {
            InitializeComponent();
            ...
            //pg_weekly.Item.IsSelected = true;
            radPageView1.SelectedPage = pg_weekly;

            optWeekly.ToggleState = ToggleState.On;

            if (advanceBookingId>0)
            {
                pg_Daily.Item.Visibility = ElementVisibility.Collapsed;
               
                optDaily.Visible = false;
                optWeekly.ToggleState = ToggleState.On;
                optWeekly.Visible = false;
                savedAdvanceBookingId = advanceBookingId;
                pnlHeader.Visible = false;
                label1.Text = "Extend MultiBooking";
                pg_weekly.Text = string.Empty;
                ...
                if (days != null)
                {
                    foreach (var item in days)
                    {
                       ...
                       else if (item.ToLower() == "tuesday")
                            chkTue.Checked = true;

                        else if (item.ToLower() == "wednesday")
                            chkWed.Checked = true;

                        else if(item.ToLower() == "thursday")
                            chkThurs.Checked = true;
                        
                        else if (item.ToLower() == "friday")
                            chkFri.Checked = true;

                        else if (item.ToLower() == "saturday")
                            chkSat.Checked = true;

                        else if (item.ToLower() == "sunday")
                            chkSun.Checked = true;
        ...
        private void InitializeFormSettings()
        {
            try
            {
                ...
                //  FormatPickupsGrid();
                grdBookings.EnableSorting = false;
        ...
        void frmMultiBooking_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if(numCompanyFareRateWeek!=null)
                    numCompanyFareRateWeek.Visible = false;


                    if (numWeekCompanyFareRateReturn != null)
                        numWeekCompanyFareRateReturn.Visible = false;


                    if ( lblweekaccfares!= null)
                        lblweekaccfares.Visible = false;



                    if (lblweekcompanyreturnfares != null)
                        lblweekcompanyreturnfares.Visible = false;
                ...
                if (startingAt != null)
                    dtpStartingAt.Value = endingAt.Value.AddDays(1).ToDate();

                if (endingAt != null)
                    dtpEndingDate.Value = endingAt.Value.AddDays(1).ToDate();
        ...
        DateTime nowDate;
        CurrentRow cr = null;
        public long savedAdvanceBookingId;
        private bool Save()
        {

            if(this.endingAt != null && dtpStartingAt.Value <= this.endingAt)
            {
                MessageBox.Show("Starting Date cannot be less or equal to " + string.Format("{0:dd/MM/yyyy HH:mm}", this.endingAt));
                return false;
            }

            if (this.endingAt != null && dtpEndingDate.Value <= this.endingAt)
            {
                MessageBox.Show("Ending Date cannot be less or equal to " + string.Format("{0:dd/MM/yyyy HH:mm}", this.endingAt));
                return false;

            }


            if(savedAdvanceBookingId>0 && grdBookings.Rows.Count==0)
            {
                MessageBox.Show("Please select bookings");
                return false;
            }
            ...
            try
            {
                ...
                endDateTime = grdBookings.Rows.Where(c=>c.Cells[COLS.DAY].Value.ToStr()!="").LastOrDefault().Cells[COLS.ReturnPickupDate].Value.ToDateTimeorNull();

                if(endDateTime==null)
                    endDateTime = grdBookings.Rows.Where(c => c.Cells[COLS.DAY].Value.ToStr() != "").LastOrDefault().Cells[COLS.PickupDate].Value.ToDateTimeorNull();
                ...
                if (savedAdvanceBookingId == 0)
                {
                    ...
                    objAdvBO.New();
                    objAdvBO.Current.AdvanceBookingNo = ObjBookiing.SubcompanyId.ToInt().ToStr();
                ...
                if(ObjBookiing!=null)
                objAdvBO.Current.OrderNo = ObjBookiing.OrderNo.ToStr();
                ...
                for (int day = 1; day <= grdBookings.RowCount; day++)
                {
                    try
                    {
                        ...
                        objMaster.IsQuotedPrice = ObjBookiing.IsQuotedPrice.ToBool();

                        objMaster.AttributeValues = ObjBookiing.AttributeValues.ToStr().Trim();
                        objMaster.ExcludedDriverIds = ObjBookiing.ExcludedDriverIds.ToStr().Trim();
                        objMaster.NoOfChilds = ObjBookiing.NoOfChilds.ToInt();
                        ...
                        objMaster.ApplyServiceCharges = ObjBookiing.ApplyServiceCharges.ToBool();

                        if(objMaster.ApplyServiceCharges.ToBool())
                        objMaster.TotalTravelledMiles = ObjBookiing.TotalTravelledMiles.ToDecimal();
                        ...
                        List<Booking_ViaLocation> listofDetail = (from r in ObjBookiing.Booking_ViaLocations.ToList()
                                                                  select new Booking_ViaLocation
                                                                  {
                                                                      ...
                                                                      ViaLocTypeId =r.ViaLocTypeId==null?Enums.LOCATION_TYPES.ADDRESS: r.ViaLocTypeId.ToIntorNull(),
                        ...
                        if (extraChargesOneWaySavedList != null)
                        {
                            ...
                            string[] skipProperties2 = { "Gen_Attribute", "Booking", "Gen_AttributesCategory" };

                            IList<Booking_Attribute> savedList3 = objMaster.Booking_Attributes;
                        ...
                        objMaster.AdvanceBookingId = savedAdvanceBookingId;
                        ...
                        if (AppVars.objPolicyConfiguration.ShowAreaWithPlots.ToBool() && AppVars.objPolicyConfiguration.EnablePDA.ToBool())
                        {
                            if (objMaster.FromPostCode.ToStr().Trim() == string.Empty)
                                ...
                                if ((LastPickupPostCode == string.Empty || LastPickupPostCode != objMaster.FromAddress.ToStr().Trim()))
                                {

                                    objMaster.ZoneId = General.GetZoneId(objMaster.FromAddress.ToStr().Trim());
                                    LastPickupPostCode = objMaster.FromAddress.ToStr().Trim();
                            ...
                            if (objMaster.ToPostCode.ToStr().Trim() == string.Empty)
                                ...
                                if ((LastDropOffPostCode == string.Empty || LastDropOffPostCode != objMaster.ToAddress.ToStr().Trim().ToUpper()))
                                {

                                    objMaster.DropOffZoneId = General.GetZoneId(objMaster.ToAddress.ToStr().Trim().ToUpper());
                                    LastDropOffPostCode = objMaster.ToAddress.ToStr().Trim().ToUpper();
                        ...
                        objMaster.JourneyTimeInMins = ObjBookiing.JourneyTimeInMins.ToDecimal();
        ...
        public decimal ReturnParking = 0.00m;
        int cnter = 0;
        private void CreateReturnJob(Booking Current,string bookingno)
        {
            ...
            try
            {
                if (Current.BookingReturns.Count == 0)
                {
                    ...
                    returnBooking.ParkingCharges = ReturnParking;
                    ...
                    returnBooking.CongtionCharges = ReturnParking;
                    ...
                    returnBooking.ServiceCharges = this.ReturnBookingFee.ToDecimal();
                }
                else
                    returnBooking = Current.BookingReturns.FirstOrDefault();

                returnBooking.ExtraMile = this.ReturnPriority.ToDecimal();
                ...
                if (this.ReturnSpecialRequirement.ToStr().Trim().Length>0)
                {

                    returnBooking.ExtraDropCharges = ReturnExtraDrop;
                }
                ...
                {
                    foreach (var itemA in extraChargesReturnSavedList)
                    {
                        returnBooking.Booking_Attributes.Add(new Booking_Attribute { AttributeId = itemA.AttributeId, Price = itemA.Price, Qty = itemA.Qty });
                    }
                ...
                returnBooking.IsQuotedPrice = Current.IsQuotedPrice.ToBool();


                returnBooking.AttributeValues = Current.AttributeValues.ToStr();
                returnBooking.ExcludedDriverIds = Current.ExcludedDriverIds.ToStr();
                returnBooking.NoOfChilds = Current.NoOfChilds.ToInt();

                if (returnBooking.CompanyId == null)
                {
                    returnBooking.TotalCharges = returnBooking.FareRate.ToDecimal()
                                 + returnBooking.ExtraDropCharges.ToDecimal() + returnBooking.MeetAndGreetCharges.ToDecimal() + returnBooking.CongtionCharges.ToDecimal()+ returnBooking.ServiceCharges.ToDecimal();
                }
                else
                {
                    returnBooking.TotalCharges = returnBooking.CompanyPrice.ToDecimal() + returnBooking.ParkingCharges.ToDecimal() + returnBooking.WaitingCharges.ToDecimal() + returnBooking.ExtraDropCharges.ToDecimal() + returnBooking.ServiceCharges.ToDecimal();
                ...
                if(Current.EscortId!=null)
                returnBooking.EscortId = Current.EscortId;
        ...
        private bool SendAdvanceBookingConfirmationText(string mobileNo)
        {
            ...
            if (enableAdvBookingText && listOfPickUpDateTime.Count > 0 && this.ObjBookiing != null && this.ObjBookiing.IsQuotation.ToBool() == false)
            {
                ...
                bool foundAny = false;
                int dayDiff = 0;
                foreach (var pickupTime in listOfPickUpDateTime.OrderBy(c => c))
                {
                 ...
                 //   minDifference = pickupTime.TimeOfDay.Subtract(nowDate.TimeOfDay).Minutes;
                     ...
                     minDifference = pickupTime.Subtract(nowDate).TotalMinutes;

                    if (dayDiff > 0 || afterMins ==0 || (afterMins>0 && minDifference >= afterMins))
                    {
                        foundAny = true;
                        break;
                    }
                ...
                if (foundAny)
                {
                    try
                    {
                        if (ObjBookiing.PickupDateTime != listOfPickUpDateTime.OrderBy(c => c).FirstOrDefault())
                        {
                            ObjBookiing.PickupDateTime = listOfPickUpDateTime.OrderBy(c => c).FirstOrDefault();
