...
namespace Taxi_AppMain
{
    public partial class frmCallHistory : UI.SetupBase
    {
        ...
        public frmCallHistory(CallerIdType_Configuration obj)
        {
            ...
            grdCalls.TableElement.Font = new Font("Tahoma", 10, FontStyle.Bold);
        }
        ...
        void grdCalls_CommandCellClick(object sender, EventArgs e)
        {
            ...
            if (row != null && row is GridViewRowInfo)
            {
                ...
                if (gridCell.ColumnInfo.Name.ToLower() == "btncreatebooking")
                {
                    ...
                    string number = row.Cells["PhoneNumber"].Value.ToStr();

                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        db.DeferredLoadingEnabled = false;

                        var objCust = db.Customers.Where(c => c.MobileNo == number || c.TelephoneNo == number).OrderByDescending(c => c.Id).FirstOrDefault();
                        ...
                        if (objCust == null)
                        {
                            ...
                            if (number.ToStr().StartsWith("07"))
                            {
                                objCust.MobileNo = number;
                                objCust.TelephoneNo = "";

                            }
                            else
                            {
                                objCust.TelephoneNo = number;
                        ...
                        General.ShowBookingForm(0, false, row.Cells[COLS.Name].Value.ToStr(), objCust.TelephoneNo.ToStr(), objCust.MobileNo.ToStr(), objCust.DoorNo.ToStr(), objCust.Address1.ToStr(), objCust.Email.ToStr(), objCust.ExcludedDriverIds.ToStr(), objCust.LikesAndDislikes.ToStr());

                        //      General.ShowBookingForm(0, false, row.Cells[COLS.Name].Value.ToStr(), row.Cells["PhoneNumber"].Value.ToStr(),objCust.MobileNo.ToStr(),objCust.DoorNo.ToStr(), objCust.Address1.ToStr(),objCust.Email.ToStr(),objCust.ExcludedDriverIds.ToStr(),objCust.LikesAndDislikes.ToStr());
                    }
        ...
        void frmCallHistory_Load(object sender, EventArgs e)
        {
            try
            {
                if (AppVars.AppTheme != "ControlDefault")
                    grdCalls.ForeColor = Color.White;
                ...
                if (this.objCLISettings.VOIPCLIType.ToInt() == 2 || this.objCLISettings.VOIPCLIType.ToInt() == 4)
                {
                  //  grdCalls.Columns["Line"].IsVisible = true;
                   // grdCalls.Columns["Duration"].IsVisible = false;
                    ...
                    if (AppVars.listUserRights.Count(c => c.functionId == "DISABLE RECORDING") == 0)
                    {
                        showRecording = true;
                        grdCalls.Columns["Recording"].IsVisible = true;
                    ...
                    {
                        grdCalls.Columns["Recording"].IsVisible = false;
                        grdCalls.Columns["Recording"].VisibleInColumnChooser = false;
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                frm.reportViewer1.ShowExportButton = btnExport.Visible;
                frm.ShowDialog();
