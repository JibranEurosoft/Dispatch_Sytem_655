                ...
                for (int i = 0; i < list2.Count; i++)
                {
                    try
                    {
                       ...
                       //     if(System.Diagnostics.Debugger.IsAttached==false)
                        ...
                        using (TaxiDataContext db = new TaxiDataContext())
                        {
                            var objPDASettings = db.Fleet_Driver_PDASettings.FirstOrDefault(x => x.Id == objPDA.Id);

                            if (objPDASettings != null)
                            {
                                objPDASettings.DisableRejectJobAuth = chkDisableJobAuth.Checked;
