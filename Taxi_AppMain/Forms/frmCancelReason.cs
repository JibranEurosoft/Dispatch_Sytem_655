...
namespace Taxi_AppMain
{
    public partial class frmCancelReason : UI.SetupBase
    {
        ...
        public frmCancelReason(long id, string cancelreason)
        {
            ...
            DisplayCancelReason();
            this.Load += FrmCancelReason_Load;
        }
        ...
        {
            if (AppVars.objPolicyConfiguration.PickCompanyAddressOnBooking.ToBool() || AppVars.listUserRights.Count(c=>c.functionId=="APPLY CANCELLATION SMS BY DEFAULT" )>0)
            {
                chkCancellationSMS.Checked = false;
                chkSendEmailtoCustomer.Checked = true;
            }
            else if(AppVars.objPolicyConfiguration.BookingFormType.ToInt()==1)
            {
                chkCancellationSMS.Checked = false;
                chkSendEmailtoCustomer.Checked = false;
            }
        }
        ...
        private void SaveBooking()
        {
            ...
            if (objMaster != null && objMaster.Current != null && objMaster.Current.BookingTypeId.ToInt() == 100)
            {
                MessageBox.Show("You cannot cancel Pool Job");
                return;
            }
        ...
        private  void CancelCurrentBookingFromPDA(long jobId, int driverId)
        {
            ...
            try
            {
                ...
                General.ReCallBookingWithStatus(jobId.ToLong(), driverId.ToInt(), Enums.BOOKINGSTATUS.CANCELLED, Enums.Driver_WORKINGSTATUS.AVAILABLE);
            ...
            //    (new TaxiDataContext()).stp_UpdateJob(jobId, driverId, Enums.BOOKINGSTATUS.CANCELLED, Enums.Driver_WORKINGSTATUS.AVAILABLE, AppVars.objPolicyConfiguration.SinBinTimer.ToInt());
               ...
               //General.SendMessageToPDA("request pda=" + driverId + "=" + jobId + "=Cancelled Job>>" + jobId + "=2").Result.ToBool();
        ...
        void frmCancelReason_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.IsRefresh == false && this.CancelReason.ToStr().Length==0)
            {
                ...
                (Application.OpenForms.OfType<Form>().FirstOrDefault(c => c.Name == "frmBookingDashBoard") as frmBookingDashBoard).UpdateNoPickupAndCancelledCountFromDb();
