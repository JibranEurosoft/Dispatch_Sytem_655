...
namespace Taxi_AppMain
{
    public partial class frmDriversList : UI.SetupBase
    {
        ...
        private void UpdatePendingApprovalValue()
        {
            try
            {
                if (AppVars.listUserRights.Count(c => c.functionId == "SHOW PENDING APPROVAL DRIVERS") > 0)
                {
                    btnPendingApproval.Visible = true;

                    int cnt = new TaxiDataContext().ExecuteQuery<int>("exec stp_GetAllFleetDriverApprovals {0}", "Pending").Count();
            ...
            try
            {
                ...
                if ((AppVars.frmMDI.ActiveControl != null && AppVars.frmMDI.ActiveControl.Name.Equals("frmDriversList1") == true))
                {
                    ...
                    foreach (var item in grdLister.Rows)
                    {
                        ...
                        if (item.Cells["MOTExpiry"].Value.ToDateTime() < dtNow)
                        {
                            item.Cells["MOTExpiry"].Style.BackColor = Color.Pink;
                            item.Cells["MOTExpiry"].Style.CustomizeFill = true;

                        }
                        else if (item.Cells["MOTExpiry"].Value.ToDateTime() < dtNow.AddDays(MOTDays))
                        {
                        ...
                        if (item.Cells["MOT2Expiry"].Value.ToDateTime() < dtNow)
                        {
                            item.Cells["MOT2Expiry"].Style.BackColor = Color.Pink;
                            item.Cells["MOT2Expiry"].Style.CustomizeFill = true;

                        }
                        else if (item.Cells["MOT2Expiry"].Value.ToDateTime() < dtNow.AddDays(MOT2Days))
                        {
                        ...
                        if (item.Cells["RoadTaxExpiry"].Value.ToDateTime() < dtNow)
                        {
                            item.Cells["RoadTaxExpiry"].Style.BackColor = Color.Pink;
                            item.Cells["RoadTaxExpiry"].Style.CustomizeFill = true;

                        }
                        else if (item.Cells["RoadTaxExpiry"].Value.ToDateTime() < dtNow.AddDays(RoadTaxDays))
                        {
                        ...
                        if (item.Cells["LicenseExpiry"].Value.ToDateTime() < dtNow)
                        {
                            item.Cells["LicenseExpiry"].Style.BackColor = Color.Pink;
                            item.Cells["LicenseExpiry"].Style.CustomizeFill = true;

                        }
                        else if (item.Cells["LicenseExpiry"].Value.ToDateTime() < dtNow.AddDays(LicenseDays))
                        {
                        ...
                        if (item.Cells["PCOVehicleExpiry"].Value.ToDateTime() < dtNow)
                        {
                            item.Cells["PCOVehicleExpiry"].Style.BackColor = Color.Pink;
                            item.Cells["PCOVehicleExpiry"].Style.CustomizeFill = true;

                        }
                        else if (item.Cells["PCOVehicleExpiry"].Value.ToDateTime() < dtNow.AddDays(PHCVehicleDays))
                        {
                        ...
                        if (item.Cells["PCODriverExpiry"].Value.ToDateTime() < dtNow)
                        {
                            item.Cells["PCODriverExpiry"].Style.BackColor = Color.Pink;
                            item.Cells["PCODriverExpiry"].Style.CustomizeFill = true;

                        }
                        else if (item.Cells["PCODriverExpiry"].Value.ToDateTime() < dtNow.AddDays(PHCDriverDays))
                        {
                    ...
                    if (btnPendingApproval.Visible)
                    {
                        if (btnPendingApproval.Tag.ToInt() > 0)
                        {
                            if (btnPendingApproval.BackColor == Color.OrangeRed)
                                btnPendingApproval.BackColor = Color.AliceBlue;
                            else
                                btnPendingApproval.BackColor = Color.OrangeRed;
                        }
                        else
        ...
        private void PopulateUI(IList list)
        {
            try
            {
                ...
                //{
                //    string teamIds = row.Cells["No"].Value.ToStr();

                //    row.Cells["No"].Value = "xxxxx";
        ...
        private void ShowUI(bool show, string message)
        {
            ...
            else
            {
                ...
                grdLister.Columns["MOTExpiry"].IsVisible = listofDocs.FirstOrDefault(c => c.DocumentName == "MOT" || c.DocumentName.ToUpper() == "MOT EXPIRY").IsVisible.ToBool();
                grdLister.Columns["MOT2Expiry"].IsVisible = listofDocs.FirstOrDefault(c => c.DocumentName == "MOT 2").DefaultIfEmpty().IsVisible.ToBool();
                grdLister.Columns["RoadTaxExpiry"].IsVisible = listofDocs.FirstOrDefault(c => c.DocumentName == "Road Tax" || c.DocumentName == "School Badge").DefaultIfEmpty().IsVisible.ToBool();
                ...
                (grdLister.Columns["MOT2Expiry"] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy HH:mm}";
                (grdLister.Columns["MOT2Expiry"] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy HH:mm";
                ///      }
                ///      
                ...
                (grdLister.Columns["RoadTaxExpiry"] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy HH:mm}";
                (grdLister.Columns["RoadTaxExpiry"] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy HH:mm";


                if (listofDocs.FirstOrDefault(c => c.DocumentName == "School Badge") != null && listofDocs.FirstOrDefault(c => c.DocumentName == "School Badge").IsVisible.ToBool())
                {
                    grdLister.Columns["RoadTaxExpiry"].HeaderText = "School Badge Expiry";
                    label9.Text = "School Badge";
                }
                ...
                (grdLister.Columns["PCODriverExpiry"] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy HH:mm}";
                (grdLister.Columns["PCODriverExpiry"] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy HH:mm";


                (grdLister.Columns["PCOVehicleExpiry"] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy HH:mm}";
                (grdLister.Columns["PCOVehicleExpiry"] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy HH:mm";

                (grdLister.Columns["MOTExpiry"] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy HH:mm}";
                (grdLister.Columns["MOTExpiry"] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy HH:mm";
