...
namespace Taxi_AppMain
{
    public partial class frmAddNewAddress : UI.SetupBase
    {
        ...
        public bool issaved;
        string savedPostcode = string.Empty;
        public frmAddNewAddress(long Id, string Address,int? zoneId,string postcode,string source)
        {
            InitializeComponent();
            EntityID = Id;
            txtAddress.Text =( Address.ToStr().Trim() + " "+ postcode).Trim();
            OldAddress = Address;
        ...
        private void SaveAddres()
        {
            try
            {
                ...
                //if (savedPostcode.ToStr().Length > 0)
                ...
                //{
                    ...
                    if(PostCode.ToStr().Length>0 && Address.Contains(PostCode))
                    Address = Address.Replace(PostCode, "");

                    Entity = db.stp_SaveGeneralAddress(EntityID, Address, PostCode, source.ToStr()).FirstOrDefault().Entity.ToLong();
                    }

                issaved = true;

                 //  if(AppVars.listOfAddress
