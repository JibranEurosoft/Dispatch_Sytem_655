...
namespace Taxi_AppMain
{
    ...
    //   private  bool IsPickEmail = false;
        ...
        public frmEmailBooking(Booking objBook,bool autoSend,string BookingActionType = "")
        {
            ...
            try
            {
                this.objBooking = objBook;
                this.objSubCompany = objBook.SubcompanyId != null ? objBook.Gen_SubCompany : AppVars.objSubCompany;
                this.IsDirectEmail = autoSend;
                string subject = string.Empty;



                if (objBook.IsQuotation.ToBool())
                {
                    ...
                    {
                        subject = "BOOKING QUOTATION - " + string.Format("{0:dd MMMM yyyy}", objBook.PickupDateTime)
                             + ", TIME " + string.Format("{0:HH.mm}", objBook.PickupDateTime) + " - BOOKING ID "
                             + objBook.BookingNo.ToStr();
                    }
                ...
                else
                {
                    ...
                    {
                        subject = "BOOKING CONFIRMATION -  " + string.Format("{0:dd MMMM yyyy}", objBook.PickupDateTime)
                             + ", TIME " + string.Format("{0:HH.mm}", objBook.PickupDateTime) + " - BOOKING ID "
                             + objBook.BookingNo.ToStr();
                    }
                ...
                string toEmail = objBook.CustomerEmail.ToStr().Trim();
                int emailType = 0;
                if (autoSend)
                {
                    if (objBook.CompanyId != null)
                    {
                        if (objBook.BookedBy.ToStr().Trim().Length > 0)
                        //&& usePassengerEmail==false)
                        {
                            if (objBook.Booking_MemberDetails != null && objBook.Booking_MemberDetails.BookerEmail.ToStr().Trim().Length > 0)
                            {
                                if (AppVars.objPolicyConfiguration.PickCompanyAddressOnBooking.ToBool())
                                {
                                    toEmail = objBook.Booking_MemberDetails.BookerEmail.ToStr().Trim();
                                }
                                else
                                    toEmail += "," + objBook.Booking_MemberDetails.BookerEmail.ToStr().Trim();
                        ...
                        if (BookingActionType.ToStr().Trim().Length == 0)
                        {



                            string companyEmail = objBooking.Gen_Company.Email.ToStr().Trim();
                ...
                if (objBook.CompanyId != null)
                {
                    if ( AppVars.objPolicyConfiguration.PickCompanyAddressOnBooking.ToBool() && AppVars.listUserRights.Count(c => c.functionId == "SHOW SETFARES") > 0)
        ...
        void frmEmailBooking_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (objBooking.IsQuotation.ToBool())
                {
                    ...
                    txtSubject.Text = "BOOKING QUOTATION - " + string.Format("{0:dd MMMM yyyy}", objBooking.PickupDateTime) + ", TIME " + string.Format("{0:HH.mm}", objBooking.PickupDateTime) + " - BOOKING ID " + objBooking.BookingNo.ToStr();
                ...
                if (obj.TemplateName.ToStr() == "Template5" || obj.TemplateName.ToStr() == "Template6" || obj.TemplateName.ToStr() == "Template7" || obj.TemplateName.ToStr() == "Template8")
                {
                   ...
                   else
                   {
                        ...
                        if (AppVars.listUserRights.Where(c => c.functionId == "PREFERRED CUSTOMER EMAIL").Count() > 0)
                ...
                if (AppVars.listUserRights.Where(c => c.functionId == "HIDE FARES ON CONFIRMATION EMAIL").Count() > 0)
                    
                {

                    chkHideFares.Visible = true;
                    chkHideFares.Checked = true;
                }


                if (
                    (AppVars.listUserRights.Where(c => c.functionId == "HIDE FARES ON ACCOUNT JOB CONFIRMATION EMAIL").Count() > 0
                           && objBooking.PaymentTypeId.ToInt() == Enums.PAYMENT_TYPES.BANK_ACCOUNT)
                           )
                {

                    chkHideFares.Visible = false;
                    chkHideFares.Checked = true;
                }
            }
        ...
        {
            SetConfirmationEmail(ddlFrom.Text.ToStr().Trim(), txtTo.Text.Trim(),false,ddlEmailToType.SelectedIndex,txtSubject.Text.Trim(),chkHideFares.Checked);
        }


        private void SetConfirmationEmail(string fromEmail,string toEmail,bool directEmail,int toEmailType,string subject,bool hideFares)
        {
            ...
            try
            {
              ...
              //  string subject = txtSubject.Text.Trim();
                ...
                else if (template == "Template6")
                {
                    ...
                    if (directEmail == false)
                    {
                        ...
                        if (toEmailType == 0)
                        {
                            if (toEmail.Contains(",") || toEmail.Contains(":"))
                            {
                                ...
                                char[] delimiterChars = { ',', ':', };
                                string[] emailarray = toEmail.Split(delimiterChars);
                    ...
                    string tel = "<h4 style=\"color:#6b97c2;  padding:0; margin:0\">" + this.objSubCompany.TelephoneNo + "<br><span style=\"color:#6b97c2;font-size:13px\">" + this.objSubCompany.Address + "</span></h4>";

                    if (AppVars.objPolicyConfiguration.DefaultClientId == "MIllenIum.ExecuTIves")
                    {
                        tel = "<h4 style=\"color:#6b97c2;  padding:0; margin:0\">24 HRS TEL: +44(0)" +  this.objSubCompany.TelephoneNo.Substring(1) + "<br><span style=\"color:#6b97c2;font-size:13px\">" + this.objSubCompany.SmtpUserName.ToStr() + "</span><br><span style=\"color:#6b97c2;font-size:13px\">" + this.objSubCompany.Address + "</span></h4>";
                    }

                    if (AppVars.objPolicyConfiguration.PickCompanyAddressOnBooking.ToBool() && this.objSubCompany.CompanyLogoOnlinePath.ToStr().Trim().Length>0)
                        StrBld.Append("<tr><td colspan='4' style='text-align: center; padding: 5px 0px 5px 0px; font-size: 18px;font-weight: normal; color: #6b97c2 align=\"Center\";'><h2 style=\"color:#6b97c2; padding:0; margin:0\">" + "<img src='" + this.objSubCompany.CompanyLogoOnlinePath.ToStr().Trim() + "' />" + tel+"</td></tr><tr><td colspan='4' style='text-align: center; padding: 5px 0px 5px 0px; font-size: 18px;font-weight: normal; color: #6b97c2 ;'><br>");
                    else                 
                    ...
                    //if (AppVars.listUserRights.Count(c => c.functionId == "ADD EXTRA MESSAGE ON EMAIL FOOTER") > 0)
                    //{
             ...
             //Template8
                    ...
                    new System.Threading.Thread(delegate ()
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                File.AppendAllText(Application.StartupPath + "\\directsendemail.txt", DateTime.Now + " :" + subject + Environment.NewLine);
                        ...
                        catch(Exception ex)
                        {
                            try
                            {
                                ...
                                File.AppendAllText(Application.StartupPath + "\\exception_directsendemail.txt", DateTime.Now + " :subject :"+ subject+ ",exception:" + ex.Message + Environment.NewLine);
                ...
                try
                {
                    ...
                    if (directEmail == true)
                    {
                        using (TaxiDataContext db = new TaxiDataContext())
                        {
                            db.stp_BookingLog(Id, AppVars.LoginObj.UserName.ToStr(), "Confirmation Email to (" + toEmail + ")");

                            SentEmail obj = new SentEmail();
                            obj.SentOn = DateTime.Now;
                            obj.SentTo = toEmail.ToStr().Trim();
                            obj.EmailBody = body;
        ...
        public void SendEmail(string fromAddress, string ToAddress, string subject, string strBody)
        {
            ...
            {
                ClsEASendEmail em = new ClsEASendEmail(fromAddress,ToAddress,subject,strBody,smtpHost,ccEmail);
                em.Send(name,  userName, pwd   );
