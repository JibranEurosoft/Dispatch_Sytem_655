    ...
    public partial class frmFetchedOnlineBookingsPopup :Form
    {
        ...
        private void RefreshDashBoardBookings()
        {
            ...
            try
            {
                ...
                General.SendMessageToPDA("request broadcast=" + "**close authorize web>>" + Environment.MachineName+">>"+this.Tag.ToStr());
        ...
        private void EmailCustomer(GridViewRowInfo row,string action)
        {
           ...
           long onlineBookingId = row.Cells["ID"].Value.ToLong();
            ...
            string auth = "Online booking declined";
            ...
            || oldFare!=fares)
            {
                ...
                if (objBO.Current != null && objBO.PrimaryKeyValue != null)
                {
                    ...
                    //    }
                    objBO.Current.OnlineBookingId = null;
                    objBO.Current.PickupDateTime = row.Cells["PICKUPDATETIME"].Value.ToDateTime();
                    objBO.Current.AutoDespatch = true;
                    objBO.Current.IsBidding = true;
                    objBO.Current.Booking_Logs.Add(new Booking_Log { BookingId = objBO.Current.Id, AfterUpdate = auth, BeforeUpdate="", UpdateDate = DateTime.Now, User = AppVars.LoginObj.UserName.ToStr() });

                    objBO.CheckDataValidation = false;
                    objBO.DisableUpdateReturnJob = true;
            ...
            else
            {
                ...
                if (objBO.Current != null && objBO.PrimaryKeyValue != null)
                {
                    ...
                    objBO.Current.AutoDespatch = true;
                    objBO.Current.IsBidding = true;
                    objBO.Current.Booking_Logs.Add(new Booking_Log { BookingId = objBO.Current.Id, BeforeUpdate="", AfterUpdate = auth, UpdateDate=DateTime.Now, User=AppVars.LoginObj.UserName.ToStr() });
           ...
           // {
            if (!string.IsNullOrEmpty(row.Cells["EMAIL"].Value.ToStr()))
            {
                ...
                new Thread(delegate()
                {
                    ...
                    {
                        SendEmail(null, email, "Your Booking Ref No: " + onlineBookingId.ToStr(), msgBody);
        ...
        private void EmailCustomer(GridViewRowInfo row,GridViewRowInfo returnRow, string action)
        {
                ...
                )
            {
                ...
                else
                {
                    ...
                    objBO.Current.AutoDespatch = true;
                    objBO.Current.IsBidding = true;
                    objBO.Current.Booking_Logs.Add(new Booking_Log { BookingId = objBO.Current.Id, AfterUpdate = auth, UpdateDate = DateTime.Now, User = AppVars.LoginObj.UserName.ToStr() });
        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ListofFetechedJobs.Count; i++)
                {
                    ...
                    this.Tag = this.Tag.ToStr() + ListofFetechedJobs[i].Id.ToStr();
