...
namespace Taxi_AppMain
{
    public partial class frmSearchDriver : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                ComboFunctions.FillDriverNoComboSorted(ddl_Driver, c => c.IsActive == true && (AppVars.DefaultDriverSubCompanyId == 0 || c.SubcompanyId == AppVars.DefaultDriverSubCompanyId));
                ...
                this.FormClosing += FrmSearchDriver_FormClosing;
            }
            catch
            {
        ...
        private void FrmSearchDriver_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        private void btnCallDriver_Click(object sender, EventArgs e)
        {
            ...
            if (AppVars.CallDriverLastTime == null || AppVars.CallDriverLastTime.Value < DateTime.Now)
            {
                AppVars.CallDriverLastTime = DateTime.Now.AddSeconds(10);
                ...
                btnCallDriver.Enabled = false;
                worker.RunWorkerAsync(ddl_Driver.SelectedValue.ToInt() + "|" + txtMobileNo.Text);

           //     General.ClickACallDriver(ddl_Driver.SelectedValue.ToInt(), txtMobileNo.Text);
            }
            else
            {
                MessageBox.Show("Please wait " + AppVars.CallDriverLastTime.Value.Subtract(DateTime.Now).TotalSeconds.ToInt() +" second(s)");
        ...
        private void Worker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                if (e.Argument != null)
                {
                    ...
                    var arr = data.Split('|');
                    e.Result = General.ClickACallDriver(arr[0].ToInt(), arr[1].ToStr().Trim());
                }
            }
            catch
            {
