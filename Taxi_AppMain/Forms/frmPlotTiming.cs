    ...
    public partial class frmPlotTiming : UI.SetupBase
    {
        ...
        private void FormatGrid()
        {
           ...
           // grdZones.AutoCellFormatting = true;
            ...
            var zoneList = AppVars.BLData.GetAll<Gen_Zone>(c => c.MinLatitude != null).OrderBy(c => c.OrderNo).Select(args => new { Id = args.Id, ZoneName = args.OrderNo + "." + args.ZoneName + " (" + args.ShortName + ")",OnlyZoneName=  args.ZoneName + " (" + args.ShortName + ")" }).ToList();

            var  backupzoneList = zoneList.OrderBy(c => c.OnlyZoneName).Select(args => new { Id = args.Id, ZoneName =args.OnlyZoneName }).ToList();
            ...
            colCombo.DataSource = backupzoneList.ToList();
          ...
          //  colCombo.DataSource = AppVars.BLData.GetAll<Gen_Zone>(c => c.MinLatitude != null).OrderBy(c => c.OrderNo).Select(args => new { Id = args.Id, ZoneName = args.OrderNo + "." + args.ZoneName + " (" + args.ShortName + ")" }).ToList();
            ...
            GridViewDecimalColumn colDD = new GridViewDecimalColumn();
            colDD.HeaderText = "Booking Fee";
            colDD.Name = "BookingFee";
            colDD.ReadOnly = false;
            colDD.Width = 130;
            colDD.Maximum = 1000;
            colDD.Minimum = -1;
            colDD.IsVisible = true;
            colDD.DecimalPlaces = 2;
            grdZones.Columns.Add(colDD);




            colDD = new GridViewDecimalColumn();
            colDD.HeaderText = "Via Charge";
            colDD.Name = "ViaCharge";
            colDD.ReadOnly = false;
            colDD.Width = 130;
            colDD.Maximum = 1000;
            colDD.Minimum = -1;
            ...
            GridViewCheckBoxColumn colCap = new GridViewCheckBoxColumn();
            colCap.HeaderText = "Capped Fare";
            colCap.Name = "CappedFare";
            colCap.ReadOnly = false;
            colCap.Width = 130;
            colCap.IsVisible = true;
            grdZones.Columns.Add(colCap);
        ...
        void frmPlotTiming_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    if (list[i].JobDueTime == null)
                    {
                        grdZones.Rows[i].Cells["JobDue"].Value = new DateTime(1753, 1, 1, 0, 0, 0);
                    }
                    ...
                    grdZones.Rows[i].Cells["CappedFare"].Value = list[i].CappedFare.ToBool();
                    grdZones.Rows[i].Cells["ViaCharge"].Value = list[i].ViaCharge.ToDecimal();
                    grdZones.Rows[i].Cells["BookingFee"].Value = list[i].BookingFee.ToDecimal();
            ...
            try
            {
                ...
                foreach (var item in grdZones.Rows.Where(c => c.Cells["IsUpdated"].Value.ToStr() == "1"))
                {
                    ...
                    if (objMaster.Current != null)
                    {
                        ...
                        objMaster.Current.CappedFare = item.Cells["CappedFare"].Value.ToBool();
                        objMaster.Current.ViaCharge = item.Cells["ViaCharge"].Value.ToDecimal();
                        objMaster.Current.BookingFee = item.Cells["BookingFee"].Value.ToDecimal();
