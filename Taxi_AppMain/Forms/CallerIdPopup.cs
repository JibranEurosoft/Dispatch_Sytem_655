    ...
    public partial class CallerIdPopup :Form
    {
        ...
        void CallerIdPopup_Shown(object sender, EventArgs e)
        {
         //   new Thread(new ThreadStart(LoadCLIHistory)).Start();
          ...
          delegate void UIDelegateIList(IList list);

        private void LoadCLIHistory(IList list)
        {

            if (this.InvokeRequired)
            {
                this.BeginInvoke(new UIDelegateIList(LoadCallHistory), list);

            }
            else
            {
                LoadCallHistory(list);
        ...
        private void LoadCallHistory(IList list)
        {
                 ...
                 //       temp.Columns["PickupDate"].IsVisible = false;
                     ...
                     //   lblUsed.Visible = true;
                      ...
                      //  lblNoFares.Text = "No Fares : " + list.FirstOrDefault().DefaultIfEmpty().NoPickupJobCount.ToInt();
                        ...
                        lblUsed.Text = "Used : " + list.Sum(c => c.Completed.ToInt());
                        lblCancelled.Text = "Cancelled : " + list.Sum(c => c.CancelledJobCount.ToInt());
                        lblNoFares.Text = "No Pickup : " + list.Sum(c => c.NoPickupJobCount.ToInt());
                    }
                    else
                    {
                        ...
                        using (TaxiDataContext db = new TaxiDataContext())
                        {
                            ...
                            if (objBooking != null)
                            {
                                ...
                                if (obj1 != null)
                                {
                                    ...
                                    string eta =  General.GetETATime(Orign.Latitude + "," + Orign.Longitude, dest.Latitude + "," + dest.Longitude, AppVars.etaKey);
