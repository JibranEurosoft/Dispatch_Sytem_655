...
{
    public partial class frmDriverCommissionPayExpenses5 : UI.SetupBase
    {
        bool IsDriverLoaded = false;
        long CommissionId = 0;
        DriverCommisionBO objDriverCommision = new DriverCommisionBO();
        public frmDriverCommissionPayExpenses5()
        {
            InitializeComponent();
            FormatGrid();
        ...
        private void Save()
        {
            try
            {
                ...
                objDriverCommissionExpense.AddBy = AppVars.LoginObj.UserName.ToStr().Trim();
                ...
                if (objDriverCommissionExpense.Description.ToStr().Trim().Length == 0)
                {
                    if (rdoCredit.IsChecked)
                    {
                        objDriverCommissionExpense.Description = "PAID TO OFFICE";
                    }
                    else
                    {
                        objDriverCommissionExpense.Description = "PAID TO DRIVER";

                    }
                    Description = objDriverCommissionExpense.Description;
                }

                if (objDriverCommissionExpense.Description.ToStr().Contains("(Payment)") == false)
                    objDriverCommissionExpense.Description = Description + " (Payment)";
        ...
        private void DisplayRecord()
        {
            ...
            try
            {
                ...
                if (item != null)
                {
                    ...
                    var list = (from a in General.GetQueryable<Fleet_DriverCommissionExpense>(c => c.CommissionId == CommissionId )
                                //where a.IsPaid == true
                                orderby a.Date 
                                select new
                                {
        ...
        private void FormatGrid()
        {
            ...
            if (AppVars.listUserRights.Count(c => c.formName == "frmDriverCommissionPayExpenses5" && c.functionId == "DELETE") == 0)
                cmd.IsVisible = false;
        ...
        {
            ComboFunctions.FillDriverNoCombo(ddlDrivers, c => c.DriverTypeId == 2 && c.IsActive==true);
        ...
        private void btnSendEmail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                long id = objDriverCommision.Current.Id;
                ...
                frm.SendEmail(objDriverCommision.Current.TransNo, objDriverCommision.Current.Fleet_Driver.DefaultIfEmpty().Email.ToStr().Trim());
            }
            catch(Exception ex)
            {
        ...
        private void btnViewReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                frm.IsFareAndWaitingWise = false;
             

                frm.objSubCompany = objDriverCommision.Current.Fleet_Driver.Gen_SubCompany;
                frm.GenerateReport();
