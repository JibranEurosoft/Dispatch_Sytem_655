...
namespace Taxi_AppMain
{
    public partial class frmSysPolicy : SetupBase
    {
        ...
        public struct COL_AIRPORTEXPIRY
        {
            ...
            public static string FREEWAITINGMINS = "FREEWAITINGMINS";
            public static string WAITINGCHARGES = "WAITINGCHARGES";
            public static string WAITINGTIMEPERSEC = "WAITINGTIMEPERSEC";
            public static string ALLOWANCETIME = "ALLOWANCETIME";
        ...
        private void InitializeFaresPanel()
        {
            ...
            this.pnlFares.Size = new System.Drawing.Size(810, 360);
            ...
            this.grdFaresSettings.Size = new System.Drawing.Size(800, 330);
        ...
        void frmSysPolicy_Shown(object sender, EventArgs e)
        {
            ...
            if (AppVars.listUserRights.Count(c => c.functionId == "VIA POINT EXTRA CHARGES") > 0)
            {
                radLabel19.Visible = true;
                numviacharges.Visible = true;
            }
        }
        ...
        private void FormatFaresGrid()
        {
            ...
            List<RadListDataItem> comboList = new List<RadListDataItem>();
            comboList.Add(new RadListDataItem("None", "0"));
            comboList.Add(new RadListDataItem("Percentage less than Amount", "1"));
            comboList.Add(new RadListDataItem("Percentage exceeds Amount", "2"));

            var list2 = (from a in comboList
                         select new
                         {
                             Name = a.Text,
                             Id = a.Value
            ...
            GridViewComboBoxColumn comboCol2 = new GridViewComboBoxColumn();
            comboCol2.DataSource = list2;
            comboCol2.DisplayMember = "Name";
            comboCol2.ValueMember = "Id";
            comboCol2.HeaderText = "Apply Amount";
            comboCol2.Width = 200;
            comboCol2.DropDownStyle = Telerik.WinControls.RadDropDownStyle.DropDown;
            comboCol2.Name = "ApplyAmount";

            grdFaresSettings.Columns.Add(comboCol2);
        ...
        private void FormatPaymentGatewayGrid()
        {
            ...
            col = new GridViewTextBoxColumn();
            col.HeaderText = "Secret";
            col.Width = 120;
            col.Name = COL_GATEWAY.APIUSERNAME;
            col.ReadOnly = true;
            col.IsVisible = false;
            grdPaymentGateway.Columns.Add(col);
            ...
            col = new GridViewTextBoxColumn();
            col.HeaderText = "Token";
            col.Width = 120;
            col.Name = COL_GATEWAY.APIPASSWORD;
            col.ReadOnly = true;
            col.IsVisible = false;
            grdPaymentGateway.Columns.Add(col);
        ...
        public override void Save()
        {
            try
            {
                ...
                if (OpenedFromCallerId == false)
                {
                    ...
                    objMaster.Current.Gen_SysPolicy_Configurations[0].WaitAndReturnDiscountType = ddlwaitandreturndiscounttype.SelectedIndex == 1 ? 1 : 0;

                    objMaster.Current.Gen_SysPolicy_Configurations[0].ViaPointExtraCharges = numviacharges.Value;
                    ...
                    int mapTypeId = 1;
                    if (optHereMap.ToggleState == ToggleState.On)
                    ...
                    objMaster.Current.Gen_SysPolicy_Configurations[0].NoPickupCharges = numNoPickupCharges.Value;
                    ...
                    if (objMaster.Current.Gen_SysPolicy_Configurations[0].EnablePeakOffPeakFares.ToBool() == false)


                    {
                        if (chkEnablePeakOffPeakTimeFare.Checked)
                            objMaster.Current.Gen_SysPolicy_Configurations[0].FareMeterType = 2;
                        else
                            objMaster.Current.Gen_SysPolicy_Configurations[0].FareMeterType = 1;
                    ...
                    objMaster.Current.Gen_SysPolicy_Configurations[0].DeadMileage = numdeadMileage.Value.ToInt();
                    objMaster.Current.Gen_SysPolicy_Configurations[0].DeadMileageType = chkDeadMileage.Checked ? 1 : 0;

                    objMaster.Current.Gen_SysPolicy_Configurations[0].RestrictMilesPOBAction = numArriveJob.Value.ToDecimal();
                    objMaster.Current.Gen_SysPolicy_Configurations[0].RestrictMilesOnSTC = numSTCJob.Value.ToDecimal();
                    ...
                    int accountType = Enums.SMSACCOUNT_TYPE.MODEMSMS;
                    ...
                    objMaster.Current.Gen_SysPolicy_Configurations[0].UseMultipleSMSGateways = false;
                    ...
                    objMaster.Current.Gen_SysPolicy_SMSConfigurations[0].SMSModemType = "tmobile";



                    // CLICK A TELL SMS
                    objMaster.Current.Gen_SysPolicy_SMSConfigurations[0].ClickSMSUserName = txtDinstartUserName.Text.Trim();
                    objMaster.Current.Gen_SysPolicy_SMSConfigurations[0].ClickSMSPassword = txtDinstarPassword.Text.Trim();
                    objMaster.Current.Gen_SysPolicy_SMSConfigurations[0].ClickSMSApiKey = txtDinstarUrl.Text.Trim();
                    objMaster.Current.Gen_SysPolicy_SMSConfigurations[0].ClickSMSSenderName = txtDinstarSenderPort.Text.Trim();
                    objMaster.Current.Gen_SysPolicy_SMSConfigurations[0].ModemSMSPortName = txtDinstarReceiverPort.Text.Trim();
                    ...
                    if (pnlFares != null)
                    {
                        ...
                        List<Gen_SysPolicy_FaresSetting> listofFares = (from a in grdFaresSettings.Rows
                                                                        select new Gen_SysPolicy_FaresSetting
                                                                        {
                                                                            ...
                                                                            VehicleTypeName = a.Cells["ApplyAmount"].Value.ToStr()
                                                                        }).ToList();
                ...
                try
                {
                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        ...
                        string setval = chkEnableViaFixedFare.Checked ? "'true'" : "'false'";


                        int cnt = db.ExecuteQuery<int>("select count(*) from appsettings where setkey='viapointmixedfare'").FirstOrDefault().ToInt();

                        if (cnt == 0)
                            db.ExecuteQuery<int>("insert into appsettings values('','viapointmixedfare'," + setval + ",NULL,'viapointmixedfare')");
                        else
                            db.ExecuteQuery<int>("update appsettings  set setval=" + setval + " where setkey='viapointmixedfare'");
                ...
                if (txtAPIKey.Enabled || txtapiHereKey.Enabled)
                {
                    ...
                    try
                    {
                        ...
                        {
                            db.CommandTimeout = 5;
                            if (txtAPIKey.Enabled)
                                db.ExecuteQuery<int>("update mapkeys set apikey='" + txtAPIKey.Text.Trim() + "' where id<6");

                            if (txtapiHereKey.Enabled)
                                db.ExecuteQuery<int>("update mapkeys set apikey='" + txtapiHereKey.Text.Trim() + "' where id=6");
                ...
                if (txtIVRNumbers.Enabled)
                {
                    string clientCode = AppVars.objSubCompany.CompanyName;
                    string ivrNumbers = txtIVRNumbers.Text.Trim();
                    string conn = Cryptography.Decrypt(System.Configuration.ConfigurationSettings.AppSettings["ConnectionString"].ToStr(), "tcloudX@@!", true);
                    conn = Cryptography.Encrypt(conn, "softeuroconnskey", true);
                    string cloudUrl = Cryptography.Encrypt(System.Configuration.ConfigurationSettings.AppSettings["huburl"].ToStr(), "tcloudX!", true);

                    if (ivrNumbers != txtIVRNumbers.Tag.ToStr() || chkCalculateFares.Checked != chkCalculateFares.Tag.ToBool() || chkReleaseMode.Checked != chkReleaseMode.Tag.ToBool())

                    {

                        IVRCaller.SaveIVRInfo(new IVRInfo { ClientName = clientCode, ClientConn = conn, CloudUrl = cloudUrl, IVRNumbers = ivrNumbers, ClientId = AppVars.objPolicyConfiguration.DefaultClientId, CalculateFares = chkCalculateFares.Checked, ReleaseMode = chkReleaseMode.Checked });
                    }
        ...
        public override void DisplayRecord()
        {
            try
            {
                if (objMaster.Current != null)
                {
                    ...
                    if (OpenedFromCallerId == false)
                    {
                        ...
                        if (objMaster.Current.Gen_SysPolicy_Configurations.Count == 1)
                        {
                            ...
                            chkDeadMileage.Checked = obj.DeadMileageType.ToInt() == 0 ? false : true;
                            ...
                            if (objMaster.Current.Gen_SysPolicy_Configurations[0].WaitAndReturnDiscountType.ToInt() == 2)
                                ddlwaitandreturndiscounttype.SelectedIndex = 1;
                            else
                                ddlwaitandreturndiscounttype.SelectedIndex = 0;
                            ...
                            if (mapType == Enums.MAP_TYPE.GOOGLEMAPS)
                                ...
                                optHereMap.ToggleState = ToggleState.On;
                            ...
                            numviacharges.Value = objMaster.Current.Gen_SysPolicy_Configurations[0].ViaPointExtraCharges.ToDecimal();
                            ...
                            numNoPickupCharges.Value = objMaster.Current.Gen_SysPolicy_Configurations[0].NoPickupCharges.ToDecimal();
                            ...
                            numArriveJob.Value = objMaster.Current.Gen_SysPolicy_Configurations[0].RestrictMilesPOBAction.ToDecimal();
                            numSTCJob.Value = objMaster.Current.Gen_SysPolicy_Configurations[0].RestrictMilesOnSTC.ToDecimal();
                        ...
                        if (objMaster.Current.Gen_SysPolicy_SMSConfigurations.Count == 1)
                        {
                            ...
                            int SMSAccountType = Enums.SMSACCOUNT_TYPE.MODEMSMS;


                            chkCanReadSMS.Checked = objSMS.CanReadSMS.ToBool();


                            txtDinstartUserName.Text = objSMS.ClickSMSUserName.ToStr().Trim();
                            txtDinstarPassword.Text = objSMS.ClickSMSPassword.ToStr().Trim();
                            txtDinstarUrl.Text = objSMS.ClickSMSApiKey.ToStr().Trim();
                            txtDinstarSenderPort.Text = objSMS.ClickSMSSenderName.ToStr().Trim();
                            txtDinstarReceiverPort.Text = objSMS.ModemSMSPortName.ToStr().Trim();
                        ...
                        foreach (var item in objMaster.Current.Gen_SysPolicy_PaymentDetails)
                        {
                            ...
                            row.Cells[COL_GATEWAY.APIUSERNAME].Value = item.ApiUsername.ToStr();
                            row.Cells[COL_GATEWAY.APIPASSWORD].Value = item.ApiPassword.ToStr();
                ...
                bool enableviafixedfare = false;
                try
                {

                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        enableviafixedfare = db.ExecuteQuery<string>("select setval from appsettings where setkey='viapointmixedfare'").FirstOrDefault().ToStr().ToBool();
                    }
                }
                catch
                {
                ...
                chkEnableViaFixedFare.Checked = enableviafixedfare;
        ...
        private void grdFaresSettings_CellBeginEdit(object sender, GridViewCellCancelEventArgs e)
        {
            try
            {
                if (e.Column != null)
                {
                    ...
                    if (e.Row.Cells["ApplyAmount"].Value.ToStr().Trim().Length > 0 && e.Row.Cells["ApplyAmount"].Value.ToStr().Trim().ToInt() > 0)
                        return;
                    else
                    {
        ...
        private void ddlGateway_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            else if (gatewayId == 2 || gatewayId == 3 || gatewayId == 4 || gatewayId == 11)
            {
            ...
            else if (gatewayId == 7)
            {
                ...
                ShowPaymentProcessUrl(true);
                lblPaymentProcessUrl.Text = "Merchant Email";
            ...
            else if (gatewayId == 15)
            {
                pnlConnectPay.Visible = false;
                pnlOtherGateway.Visible = false;
                pnlPaypal.Visible = false;
                PnlKOnnectPay.Visible = true;

                RdoPreAuthKPCustomer.Visible = false;
                RdoDirectKPCustomer.Visible = false;
                lblKPCustomerApp.Visible = false;
                if (KonnectPaymentOptions.Items.Count <= 0)
                {
                    if (KonnectPaymentOptions.Items.Count <= 0)
                    {
                        List<KPOptions> KPOptions = GetKonnectPayOptions();
                        if (KPOptions != null && KPOptions.ToStr().Length > 0)
                        {
                            foreach (var item in KPOptions)
                            {
                                if (item.OptionType.ToStr().ToLower() == "dispatch")
                                {
                                    KonnectPaymentOptions.Items.Add(item.OptionName);
                                }
                                else if (item.OptionType.ToStr().ToLower() == "driverapp")
                                {
                                    KPOptionsDriverApp.Items.Add(item.OptionName);

                                }
                                //else if (item.OptionType.ToStr().ToLower() == "customerapp")
                                //{
                                //    RdoPreAuthKPCustomer.Visible = true;
                                //    RdoPreAuthKPCustomer.Checked = true;
                                //    RdoDirectKPCustomer.Visible = true;
                                //    lblKPCustomerApp.Visible = true;

                                //}
                                else if (item.OptionType.ToStr().ToLower() == "customerapp")
                                {
                                    if (item.OptionName.ToStr().ToLower().Contains("pre authorize"))
                                    {
                                        RdoPreAuthKPCustomer.Visible = true;
                                        RdoPreAuthKPCustomer.Checked = true;
                                        lblKPCustomerApp.Visible = true;
                                    }
                                    if (item.OptionName.ToStr().ToLower().Contains("pay now"))
                                    {
                                        RdoDirectKPCustomer.Visible = true;
                                        lblKPCustomerApp.Visible = true;
                                        if (!RdoPreAuthKPCustomer.Visible)
                                        {
                                            RdoDirectKPCustomer.Checked = true;
                                        }
                                    }
        ...
        private List<KPOptions> GetKonnectPayOptions()
        {
            List<KPOptions> KPOptions = new List<KPOptions>();
            using (TaxiDataContext db = new TaxiDataContext())
            {
                KPOptions = db.ExecuteQuery<KPOptions>("select OptionID,OptionName,OptionType from Gen_KonnectPayOptions").ToList();

            }
            return KPOptions;

        }
        public class KPOptions
        {
        ...
        private void btnAddGateway_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string apiusername3dsecret = txtJudo3DSecret.Text.Trim();
                string apipassword3dtoken = txtJudo3DToken.Text.Trim();
                ...
                if (gatewayId == 11 || gatewayId == 7)
                {
                    if (optPP_Live.IsChecked)
                        ppAccountType = 0;
                    else if (optPP_Sandbox.IsChecked)
                        ppAccountType = 1;
                    else if (optpp_paybylink.IsChecked)
                        ppAccountType = 2;

                    //  ppAccountType =optPP_Sandbox.IsChecked?1:0;
                }
                ...
                else if (gatewayId == 9) // SUMUP
                {
                    paypalid = txtInstId.Text;
                    if (paypalid.ToStr().Trim().Length == 0)
                        error += "Required : GatewayID/Affiliate Key";

                }
                else if (gatewayId == 15) // KOnnect Pay
                {
                    paypalid = TxtAccountIDKP.Text;
                    if (paypalid.ToStr().Trim().Length == 0)
                        error += "Required : Account ID";

                    paymentProcessUrl = TxtSecretKeyKP.Text.Trim();

                    //SecretKeyKP
                    if (string.IsNullOrEmpty(paymentProcessUrl))
                    {
                        error += "Required Country ID" + Environment.NewLine;

                    }
                    try
                    {
                        //Get OnlineBooking Customer app Selected Option - start
                        string KP_OptionsCustomerApp = string.Empty;

                        if (RdoPreAuthKPCustomer.Checked)
                        {
                            KP_OptionsCustomerApp = "Pre Authorize";
                        }
                        else if (RdoDirectKPCustomer.Checked)
                        {
                            KP_OptionsCustomerApp = "Pay Now";
                        }
                        //Get OnlineBooking Customer app Selected Option - end

                        //Get Dispatch Selected Option - start
                        string KP_OptionsDispatch = string.Empty;
                        var checkedOptionsDispatch = new System.Collections.Generic.List<string>();
                        for (int i = 0; i < KonnectPaymentOptions.Items.Count; i++)
                        {
                            if (KonnectPaymentOptions.GetItemChecked(i))
                            {
                                checkedOptionsDispatch.Add(KonnectPaymentOptions.Items[i].ToString());
                            }
                        }
                        KP_OptionsDispatch = string.Join(",", checkedOptionsDispatch);
                        //Get Dispatch Selected Option - end

                        //Get Driver app Selected Option - start
                        string KP_OptionsDriverApp = string.Empty;
                        var checkedOptionsDriverApp = new System.Collections.Generic.List<string>();

                        for (int i = 0; i < KPOptionsDriverApp.Items.Count; i++)
                        {
                            if (KPOptionsDriverApp.GetItemChecked(i))
                            {
                                checkedOptionsDriverApp.Add(KPOptionsDriverApp.Items[i].ToString());
                            }
                        }
                        KP_OptionsDriverApp = string.Join(",", checkedOptionsDriverApp);
                        //Get Driver app Selected Option - end
                        merchantPass = KP_OptionsCustomerApp + Environment.NewLine + KP_OptionsDispatch + Environment.NewLine + KP_OptionsDriverApp;
                    }
                    catch { }
                ...
                row.Cells[COL_GATEWAY.APIUSERNAME].Value = apiusername3dsecret;
                row.Cells[COL_GATEWAY.APIPASSWORD].Value = apipassword3dtoken;
        ...
        private void ClearGatewayDetails()
        {
            ...
            txtJudo3DSecret.Text = string.Empty;
            txtJudo3DToken.Text = string.Empty;
            ...
            try
            {
                if (grdPaymentGateway.CurrentRow != null && grdPaymentGateway.CurrentRow is GridViewDataRowInfo)
                {
                    ...
                    if (ddlGateway.SelectedValue.ToInt() == 6)
                    {
                        ...
                        txtJudo3DSecret.Text = grdPaymentGateway.CurrentRow.Cells[COL_GATEWAY.APIUSERNAME].Value.ToStr();
                        txtJudo3DToken.Text = grdPaymentGateway.CurrentRow.Cells[COL_GATEWAY.APIPASSWORD].Value.ToStr();
                        groupBox1.Visible = true;
                        groupBox2.Visible = true;
                    }
                    else if (ddlGateway.SelectedValue.ToInt() == 9)
                    {
                        pnlOtherGateway.Visible = true;
                        pnlConnectPay.Visible = false;
                        pnlPaypal.Visible = false;
                    ...
                    else if (ddlGateway.SelectedValue.ToInt() == 15)
                    {
                        TxtAccountIDKP.Enabled = false;
                        TxtSecretKeyKP.Enabled = false;
                        TxtAccountIDKP.Text = grdPaymentGateway.CurrentRow.Cells[COL_GATEWAY.PAYPALID].Value.ToStr();
                        TxtSecretKeyKP.Text = grdPaymentGateway.CurrentRow.Cells[COL_GATEWAY.APPLICATIONID].Value.ToStr();
                        TxtAccountIDKP.Visible = true;
                        TxtSecretKeyKP.Visible = true;

                        string PaymentOptions = grdPaymentGateway.CurrentRow.Cells[COL_GATEWAY.MERCHANTPASSWORD].Value.ToStr();
                        try
                        {
                            if (!string.IsNullOrEmpty(PaymentOptions))
                            {
                                var KPOptionsList = PaymentOptions.Split(new[] { Environment.NewLine, "\n" }, StringSplitOptions.None);
                                if (KPOptionsList != null)
                                {
                                    string CustomerApp = KPOptionsList[0];
                                    if (!string.IsNullOrEmpty(CustomerApp) && CustomerApp.ToLower().Contains("pre authorize")) { RdoPreAuthKPCustomer.Checked = true; }
                                    if (!string.IsNullOrEmpty(CustomerApp) && CustomerApp.ToLower().Contains("pay now")) { RdoDirectKPCustomer.Checked = true; }

                                    var OptArrDispatch = KPOptionsList[1].Split(',');
                                    var OptArrDriverApp = KPOptionsList[2].Split(',');
                                    if (OptArrDispatch != null)
                                    {
                                        for (int i = 0; i < KonnectPaymentOptions.Items.Count; i++)
                                        {
                                            string listItem = KonnectPaymentOptions.Items[i].ToString().ToLower();
                                            List<string> AddedOptions = OptArrDispatch.Select(s => s.ToLower()).ToList();
                                            if (AddedOptions.Contains(listItem))
                                            {
                                                KonnectPaymentOptions.SetItemChecked(i, true);
                                            }
                                            else
                                            {
                                                KonnectPaymentOptions.SetItemChecked(i, false);
                                            }
                                        }

                                    }
                                    if (OptArrDriverApp != null)
                                    {

                                        for (int i = 0; i < KPOptionsDriverApp.Items.Count; i++)
                                        {
                                            string listItem = KPOptionsDriverApp.Items[i].ToString().ToLower();
                                            List<string> AddedOptions = OptArrDriverApp.Select(s => s.ToLower()).ToList();
                                            if (AddedOptions.Contains(listItem))
                                            {
                                                KPOptionsDriverApp.SetItemChecked(i, true);
                                            }
                                            else
                                            {
                                                KPOptionsDriverApp.SetItemChecked(i, false);
                                            }
                                        }
                    ...
                    if (accType == "1")
                    {
                        ...
                        optpp_paybylink.ToggleState = ToggleState.On;
                    else
                        optPP_Live.ToggleState = ToggleState.On;



                    ShowPaymentProcessUrl(ddlGateway.SelectedValue.ToInt() == 4 || ddlGateway.SelectedValue.ToInt() == 7);
        ...
        private void radPageView1_SelectedPageChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (radPageView1.SelectedPage == pg_callerid)
                {
                    ...
                    if (ivr != null)
                    {
                        ...
                        btnClearVoipFields.Visible = false;
                    }
                }
                else
                {
        ...
        private void DisplayFaresSettings()
        {
            ...
            foreach (var item in objMaster.Current.Gen_SysPolicy_FaresSettings.Where(c => c.IsCompanyWise == null || c.IsCompanyWise == false))
            {
                ...
                if (row != null)
                {
                    ...
                    row.Cells["ApplyAmount"].Value = item.VehicleTypeName.ToStr();
        ...
        private void opt_emerald_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            ...
            if (args.ToggleState == ToggleState.On)
            {
                ...
                lblIVRNumbers.Visible = true;
                txtIVRNumbers.Visible = true;
            ...
            else
            {
                ...
                lblIVRNumbers.Visible = false;
                txtIVRNumbers.Visible = false;
        ...
        private void UpdateCreditCardChargesType()
        {
        ...
        private void SetAutoDespatchRuleConfig()
        {
            ...
            btnCustomAutoRule.Visible = false;
            ...
            else if (optAutoDespRule5.ToggleState == ToggleState.On)
            {
                ...
                txtAutoDespDescription.Text = "Rule X => Normal/Quite/Busy Mode";
                btnCustomAutoRule.Visible = true;
                btnCustomAutoRule.BringToFront();
            }
        ...
        private void txtListenerIP_TextChanged(object sender, EventArgs e)
        {
        ...
        private void btnEditAPIKey_Click(object sender, EventArgs e)
        {
            try
            {
                frmAdminPwd frm = new frmAdminPwd("cab1433supp");
                frm.StartPosition = FormStartPosition.CenterScreen;
                frm.ShowDialog();
                if (frm.ReturnValue1.ToStr().ToLower() == "ok")
                {
                    txtAPIKey.Enabled = true;
                    string key = string.Empty;
                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        key = db.ExecuteQuery<string>("select apikey from mapkeys where maptype='google'").FirstOrDefault().ToStr();
        ...
        private void btnEdithereAPIKey_Click(object sender, EventArgs e)
        {
            try
            {
                frmAdminPwd frm = new frmAdminPwd("cab1433supp");
                ...
                if (frm.ReturnValue1.ToStr().ToLower() == "ok")
                {
                    txtapiHereKey.Enabled = true;
                    string key = string.Empty;
                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        key = db.ExecuteQuery<string>("select apikey from mapkeys where maptype='here'").FirstOrDefault().ToStr();


                    }

                    txtapiHereKey.Text = key;
                }
                else
                    MessageBox.Show("Incorrect password");
        ...
        private void chkDeadMileage_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                numdeadMileage.Value = 0;
                numdeadMileage.Enabled = false;
            }
            else
            {

                numdeadMileage.Enabled = true;
            }
        }
        ...
        private void btnRestartSMSService_Click(object sender, EventArgs e)
        {

            try
            {

                if (txtDinstartUserName.Text.Trim().Length == 0)
                {
                    MessageBox.Show("Required : User Name");
                    return;

                }
                if (txtDinstarPassword.Text.Trim().Length == 0)
                {
                ...
                if (txtDinstarUrl.Text.Trim().Length == 0)
                {
                    MessageBox.Show("Required : Url");
                    return;

                }

                if (txtDinstarSenderPort.Text.Trim().Length == 0)
                {
                    MessageBox.Show("Required : Sender Port");
                    return;

                }

                if (txtDinstarReceiverPort.Text.Trim().Length == 0)
                {
                    MessageBox.Show("Required : Receiver Port");
                    return;
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    var objConfig = db.Gen_SysPolicy_SMSConfigurations.FirstOrDefault();
                    if (objConfig == null)
                        objConfig = new Gen_SysPolicy_SMSConfiguration();



                    objConfig.ClickSMSUserName = txtDinstartUserName.Text.Trim();
                    objConfig.ClickSMSPassword = txtDinstarPassword.Text.Trim();
                    objConfig.ClickSMSApiKey = txtDinstarUrl.Text.Trim();
                    objConfig.ClickSMSSenderName = txtDinstarSenderPort.Text.Trim();
                    objConfig.ModemSMSPortName = txtDinstarReceiverPort.Text.Trim();



                    if (objConfig.Id == 0)
                        db.Gen_SysPolicy_SMSConfigurations.InsertOnSubmit(objConfig);

                    db.SubmitChanges();
                }

                //btnRestartSMSService.Text = "Restarting...";
                btnRestartSMSService.Enabled = false;

                string response = RestartSMS(System.Configuration.ConfigurationManager.AppSettings["huburl"].ToStr());
                MessageBox.Show(response);

                //   btnRestartSMSService.Text = "Restart";
                btnRestartSMSService.Enabled = true;
        ...
        private string RestartSMS(string ServerURI)
        {
            string empty = string.Empty;
            Dictionary<string, string> strs = new Dictionary<string, string>()
            {
                { "SignalRClientsType", "2" },
                { "SignalRUserType", "2" },
                { "SignalRClientDomainId", "999955" }
            };
            Microsoft.AspNet.SignalR.Client.HubConnection hubConnection = new Microsoft.AspNet.SignalR.Client.HubConnection(ServerURI, strs)
            {
                TransportConnectTimeout = new TimeSpan(8, 0, 0)
            };
            Microsoft.AspNet.SignalR.Client.IHubProxy hubProxy = hubConnection.CreateHubProxy("DispatchHub");
            try
            {
                try
                {
                    hubConnection.Start().Wait();
                    empty = hubProxy.Invoke<string>("RestartSMSService", new object[] { "restart" }).Result;
                }
        ...
        private void btnCustomAutoRule_Click(object sender, EventArgs e)
        {

            frmAutoDispatchSettings frmauto = new frmAutoDispatchSettings();
            frmauto.ShowDialog();
            frmauto.Dispose();
        }

        private void optGoogleMap_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
        ...
        private void optPP_Live_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (ddlGateway.SelectedValue.ToInt() == Enums.PAYMENT_GATEWAY.STRIPE)
            {
                if (args.ToggleState == ToggleState.On)
                {
                    radLabel94.Text = "Secret Key";
                    lblPaymentProcessUrl.Text = "Merchant Email";

                }
                else
                {
                    lblPaymentProcessUrl.Text = "Published Key";
                    radLabel94.Text = "Secret Key";
                }
