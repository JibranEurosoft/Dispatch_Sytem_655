        ...
        private void DisplayPDASettingsPanel()
        {
            if (objMaster.Current != null && objMaster.PrimaryKeyValue != null)
            {
                ...
                if (objMaster.Current.Fleet_Driver_PDASettings.Count > 0)
                {
                    ...
                    chkEnableDriverConnect.Checked = objMaster.Current.Fleet_Driver_PDASettings[0].EnableDriverConnect.ToBool();

                    chkShowDestAfterPOB.Checked = objMaster.Current.Fleet_Driver_PDASettings[0].ShowDestinationAfterPOB.ToBool();
       ...
       //     ddlSubCompany.SelectedIndexChanged += new EventHandler(ddlSubCompany_SelectedIndexChanged);
        ...
        private void LoadDocumentOnCloud(GridViewRowInfo row)
        {
            ...
            if (DialogResult.OK == openFileDialog1.ShowDialog())
            {
                ...
                string fileNamePrefix = txtDriverNo.Text.Trim();
                ...
                General.payload res = General.UploadFile(fileNamePrefix, filePath);
        ...
        private void LoadDocument(string path)
        {
            ...
            if (DialogResult.OK == openFileDialog1.ShowDialog())
            {
                ...
                if (AppVars.listUserRights.Count(c => c.functionId == "UPLOAD DOCUMENT ON CLOUD") > 0)
                {
                    ...
                    if (res.message.ToStr().ToUpper() == "SUCCESS")
                    {
                        txtLogBookDocPath.Tag = res.filePath.ToStr().Trim().Replace("\"", "");
                        txtLogBookDocPath.Text = fileName;
                        //    row.Cells[COL_DOCUMENT.FILENAME].Value = fileName;
        ...
        public override void OnNew()
        {
            ...
            try
            {
                if (pg_Shifts.Item.Visibility == ElementVisibility.Visible)
                {
                    foreach (var item in grdDriverShifts.Rows)
                    {
                        item.Cells["Check"].Value = false;
                        item.Cells[COL_SHIFT.ID].Value = 0;
                ...
                else if (radPageViewPage2.Item.Visibility == ElementVisibility.Visible)
                {
                    if (grdShifts != null)
        ...
        public override void Save()
        {
            ...
            try
            {
                ...
                if (txtLogBookDocPath.Tag.ToStr().Trim().StartsWith("http:"))
                {
                    objMaster.Current.Gender = txtLogBookDocPath.Tag.ToStr().Trim();
                }
                ...
                objMaster.Current.LicenseExpiryDate = dtpVehAssignedOn.Value.ToDateTimeorNull();

                //MaxCommission
                objMaster.Current.MaxCommission = numMaxCommission.Value.ToDecimal();

                objMaster.Current.NICNo = numMinComm.Value.ToStr();
                ...
                foreach (var row in grdDocuments.Rows)
                {
                    ...
                    {
                        objMaster.Current.PCODriverExpiryDate = row.Cells[COL_DOCUMENT.EXPIRYDATE].Value.ToDateTimeorNull();

                    }
                    else if (row.Cells[COL_DOCUMENT.DOCUMENTTITLEID].Value.ToInt() == DRIVER_DOCUMENTS.MOT)
                    {
                        objMaster.Current.MOTExpiryDate = row.Cells[COL_DOCUMENT.EXPIRYDATE].Value.ToDateTimeorNull();

                    }
                    else if (row.Cells[COL_DOCUMENT.DOCUMENTTITLEID].Value.ToInt() == DRIVER_DOCUMENTS.PCOVehicle)
                    {
                        objMaster.Current.PCOVehicleExpiryDate = row.Cells[COL_DOCUMENT.EXPIRYDATE].Value.ToDateTimeorNull();
                    ...
                    {
                        objMaster.Current.MOT2ExpiryDate = row.Cells[COL_DOCUMENT.EXPIRYDATE].Value.ToDateTimeorNull();
                    ...
                    {
                        objMaster.Current.DrivingLicenseExpiryDate = row.Cells[COL_DOCUMENT.EXPIRYDATE].Value.ToDateTimeorNull();
                ...
                //                                                ExpiryDate = r.Cells[COL_DOCUMENT.EXPIRYDATE].Value.ToDateTimeorNull(),
                ...
                if (radPanel2 != null && radPageViewPage2.Item.Visibility == ElementVisibility.Visible)
                {
                ...
                if (pg_Shifts.Item.Visibility == ElementVisibility.Visible && grdDriverShifts != null)
                {
        ...
        public override void DisplayRecord()
        {
            ...
            try
            {
                ...
                if (objMaster.Current.Gender.ToStr().Trim().StartsWith("http:"))
                {
                    try
                    {
                        if (objMaster.Current.Gender.Contains("/Document"))
                        {

                            txtLogBookDocPath.Text = objMaster.Current.Gender.ToStr().Trim().Substring(objMaster.Current.Gender.ToStr().Trim().IndexOf("/Document") + 1);

                        }
                        txtLogBookDocPath.Tag = objMaster.Current.Gender.ToStr().Trim();
                    }
                    catch
                    {
                ...
                if (objMaster.Current.NICNo.ToStr().Trim().IsNumeric())
                    numMinComm.Value = objMaster.Current.NICNo.ToStr().ToDecimal();
                else
                    numMinComm.Value = 0;
                    ...
                    try
                    {
                        if (txtAffiliateKey != null)
                        {
                            using (TaxiDataContext db = new TaxiDataContext())
                            {
                                try
                                {
                                    txtAffiliateKey.Text = db.ExecuteQuery<string>("select AffiliateKey from Fleet_Driver_DeviceInfo where driverid=" + objMaster.Current.Id).FirstOrDefault();
                ...
                foreach (GridViewRowInfo Grow in grdDocuments.Rows)
                {
                    ...
                    if (doc != null)
                    {
                        ...
                        Grow.Cells[COL_DOCUMENT.EXPIRYDATE].Value = doc.ExpiryDate.ToDateTimeorNull();
                    }
                }
        ...
        
        {
            ...
            {
                ViewDocument(txtLogBookDocPath.Tag.ToStr().Trim());
            }
            else
...
#region TextChanged
        ...
        private void chkAccComm_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if(args.ToggleState== ToggleState.On)
            {
                ...
                numAccComm.Visible = true;
            }
            else
            {
                lblAccComm.Visible = false;
                numAccComm.Visible = false;
