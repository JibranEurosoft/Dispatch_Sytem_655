...
namespace Taxi_AppMain
{
    public partial class frmSpecialDayFares : UI.SetupBase
    {
        ...
        List<Fare_ZoneWisePricing> CopyFareZoneWisePricing;
        List<Gen_Company> globalcompanyidfilter = new List<Gen_Company>();
        public frmSpecialDayFares()
        {
            InitializeComponent();

            ShowPlotWiseMileageTariff = AppVars.listUserRights.Count(c => c.functionId == "ENABLE PLOT WISE MILEAGE TARIFF") > 0;
            ...
            this.grdDetails.CommandCellClick += GrdDetails_CommandCellClick;
        ...
        private void GrdDetails_CommandCellClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int Id = grdDetails.CurrentRow.Cells["Id"].Value.ToInt();

                    if (Id > 0)
                    {

                        decimal rate = grdDetails.CurrentRow.Cells[COLS_DETAILS.FARE].Value.ToDecimal();
                        decimal companyRate = grdDetails.CurrentRow.Cells[COLS_DETAILS.COMPANYFARE].Value.ToDecimal();



                        using (TaxiDataContext db = new TaxiDataContext())
                        {
                            db.ExecuteQuery<int>("update Fare_ChargesDetails set rate=" + rate + ", CompanyRate=" + companyRate + ",PeakTimeRate=" + grdDetails.CurrentRow.Cells[COLS_DETAILS.AGENTCOMM].Value.ToDecimal() + ", OffPeakTimeRate=" + grdDetails.CurrentRow.Cells[COLS_DETAILS.AGENTCHARGE].Value.ToDecimal() + " where id=" + Id);
                ...
                else if (gridCell.ColumnInfo.Name.ToLower() == "coldelete")
                {
        ...
        private void PasteFares()
        {
            try
            {
                ...
                if (copyFareId > 0 && GlobalFareId > 0)
                {
                    ...
                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        ...
                        db.ExecuteQuery<int>("exec stp_copytariffForPlots {0},{1}", copyFareId, GlobalFareId);
        ...
        void grdPDAOtherCharges_CommandCellClick(object sender, EventArgs e)
        {
            ...
            else if (gridCell.ColumnInfo.Name.ToLower() == "coledit")
            {
                ...
                numCostRate.Value = row.Cells[COLS_OTHERDETAILS.COMPANYFARE].Value.ToDecimal();
            }
        ...
        private void EditFaresDetail()
        {
            try
            {
                ...
                if (grdOtherCharges.CurrentRow != null && grdOtherCharges.CurrentRow is GridViewDataRowInfo)
                {
                    ...
                    numCostRateStart.Value = row.Cells[COLS_OTHERDETAILS.CostRate].Value.ToDecimal();
                    ...
                    if (row.Cells[COLS_OTHERDETAILS.ZoneId].Value.ToInt() > 0)
                    {
                        ddlZone.SelectedValue = row.Cells[COLS_OTHERDETAILS.ZoneId].Value.ToInt();
                    }
                    else
                    {
                    ...
                    numwaitingcharges.Value = row.Cells[COLS_OTHERDETAILS.WaitingCharges].Value.ToDecimal();
                    numwaitingfreemins.Value = row.Cells[COLS_OTHERDETAILS.WaitingChargesfree].Value.ToDecimal();
                    numwaitingsecs.Value = row.Cells[COLS_OTHERDETAILS.WaitingChargessecs].Value.ToDecimal();
                }
            }
            catch (Exception ex)
            {
        ...
        private bool ShowPlotWiseMileageTariff = false;

        void frmSpecialDayFares_Load(object sender, EventArgs e)
        {


            ddlZone.Visible = ShowPlotWiseMileageTariff;
            radLabel16.Visible = ddlZone.Visible;
            ...
            this.FormClosed += FrmSpecialDayFares_FormClosed;


            pnlWaitingCharges.Visible = AppVars.listUserRights.Count(c => c.functionId == "SHOW TARIFF WAITING CHARGES") > 0;




            ShowAgentDetails(chkCompanyWise.Checked);


        }

        private void FrmSpecialDayFares_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (AppVars.objPolicyConfiguration.EnableZoneWiseFares.ToBool())
            {
                GC.Collect();
        ...
        private void AddPDAOtherChargesDetail()
        {
            try
            {
                ...
                decimal companyrate = numCostRate.Value.ToDecimal();
                string msg = string.Empty;
        ...
        public struct COLS_OTHERDETAILS
        {
            ...
            public static string WaitingChargesfree = "WaitingChargesfree";
            public static string WaitingChargessecs = "WaitingChargessecs";
        ...
        private void FormatFaresOtherDetailGrid()
        {
            try
            {
                ...
                colDec.DecimalPlaces = 2;
                colDec.ThousandsSeparator = true;
                colDec.Name = COLS_OTHERDETAILS.CostRate;
                grdOtherCharges.Columns.Add(colDec);



                colDec = new GridViewDecimalColumn();
                colDec.HeaderText = "Waiting Charge";
                colDec.Width = 60;
                colDec.ReadOnly = true;
                colDec.DecimalPlaces = 2;
                colDec.ThousandsSeparator = true;
                colDec.Name = COLS_OTHERDETAILS.WaitingCharges;
                colDec.IsVisible = false;
                colDec.VisibleInColumnChooser = false;
                grdOtherCharges.Columns.Add(colDec);



                colDec = new GridViewDecimalColumn();
                colDec.HeaderText = "Waiting Free";
                colDec.Width = 50;
                colDec.ReadOnly = true;
                colDec.DecimalPlaces = 0;
                colDec.ThousandsSeparator = true;
                colDec.Name = COLS_OTHERDETAILS.WaitingChargesfree;
                colDec.IsVisible = false;
                colDec.VisibleInColumnChooser = false;
                grdOtherCharges.Columns.Add(colDec);
                ...
                colDec = new GridViewDecimalColumn();
                colDec.HeaderText = "Waiting Sec(s)";
                colDec.Width = 60;
                colDec.ReadOnly = true;
                colDec.DecimalPlaces = 2;
                colDec.ThousandsSeparator = true;
                colDec.Name = COLS_OTHERDETAILS.WaitingChargessecs;
                colDec.IsVisible = false;
                colDec.VisibleInColumnChooser = false;
                grdOtherCharges.Columns.Add(colDec);
                ...
                col = new GridViewTextBoxColumn();
                col.Name = COLS_OTHERDETAILS.ZoneId;
                col.IsVisible = false;
                grdOtherCharges.Columns.Add(col);

                col = new GridViewTextBoxColumn();
                col.Name = COLS_OTHERDETAILS.ZoneName;
                col.HeaderText = "Zone Name";
                col.IsVisible = ShowPlotWiseMileageTariff;
                col.Width = 120;
                grdOtherCharges.Columns.Add(col);
        ...
        string input_values = string.Empty;
            ...
            using (TaxiDataContext db = new TaxiDataContext())
            {
                try
                {
                    ...
                    for (int i = 0; i < grdOtherCharges.RowCount; i++)
                    {
                        ...
                        grdOtherCharges.Rows[i].Cells[COLS_OTHERDETAILS.CostRate].Value = obj[i].PerMinJourneyCharges.ToDecimal();


                        grdOtherCharges.Rows[i].Cells[COLS_OTHERDETAILS.WaitingCharges].Value = obj[i].WaitingCharges.ToDecimal();
                        grdOtherCharges.Rows[i].Cells[COLS_OTHERDETAILS.WaitingChargesfree].Value = obj[i].WaitingSecondsFree.ToDecimal();
                        grdOtherCharges.Rows[i].Cells[COLS_OTHERDETAILS.WaitingChargessecs].Value = obj[i].WaitingChargesPerSeconds.ToDecimal();

                        DateTime? fromStartTime = obj[i].SpecialDayName == "" ? obj[i].FromDateTime : obj[i].FromSpecialDate;
                        DateTime? tillStartTime = obj[i].SpecialDayName == "" ? obj[i].TillDateTime : obj[i].TillSpecialDate;
        ...
        private void AddFaresDetail()
        {
            try
            {
                decimal startRateCost = numCostRateStart.Value.ToDecimal();
                ...
                try
                {
                    ...
                    objMaster.Current.PerMinJourneyCharges = startRateCost;
                    objMaster.Current.WaitingCharges = numwaitingcharges.Value;
                    objMaster.Current.WaitingChargesPerSeconds = numwaitingsecs.Value.ToInt();
                    objMaster.Current.WaitingSecondsFree = numwaitingfreemins.Value.ToInt();
                    ...
                    try
                    {
                        ...
                        if (ddlZone.Visible)
                        {
                            ...
                            using (TaxiDataContext db = new TaxiDataContext())
                            {
                                db.ExecuteQuery<int>("Update Fare set ZoneId = " + zoneId + " where Id = " + objMaster.Current.Id + " ");
                            }
                        }
                    }
                    catch
                ...
                row.Cells[COLS_OTHERDETAILS.CostRate].Value = startRateCost;
                row.Cells[COLS_OTHERDETAILS.StartRateMile].Value = StartRateMile;



                row.Cells[COLS_OTHERDETAILS.WaitingCharges].Value = objMaster.Current.WaitingCharges.ToDecimal();
                row.Cells[COLS_OTHERDETAILS.WaitingChargesfree].Value = objMaster.Current.WaitingSecondsFree.ToDecimal();
                row.Cells[COLS_OTHERDETAILS.WaitingChargessecs].Value = objMaster.Current.WaitingChargesPerSeconds.ToDecimal();
                ...
                //
                row.Cells[COLS_OTHERDETAILS.ZoneId].Value = ddlZone.SelectedValue.ToIntorNull();
                row.Cells[COLS_OTHERDETAILS.ZoneName].Value = ddlZone.Text;
        ...
        public struct COLS_DETAILS
        {
            ...
            public static string AGENTCHARGE = "AGENTCHARGE";
        ...
        private void FormatFaresDetailGrid()
        {
            ...
            colDec = new GridViewDecimalColumn();
            colDec.HeaderText = "Hotel Comm.";
            colDec.Width = 110;
            colDec.ReadOnly = false;
            colDec.DecimalPlaces = 2;
            colDec.ThousandsSeparator = true;
            colDec.Name = COLS_DETAILS.AGENTCOMM;
            grdDetails.Columns.Add(colDec);



            colDec = new GridViewDecimalColumn();
            colDec.HeaderText = "Office Charges";
            colDec.Width = 110;
            colDec.ReadOnly = false;
            colDec.DecimalPlaces = 2;
            colDec.ThousandsSeparator = true;
            colDec.Name = COLS_DETAILS.AGENTCHARGE;
            grdDetails.Columns.Add(colDec);
            ...
            AddCommandColumn(grdDetails, "btnupdate", "Update");
            AddCommandColumn(grdDetails, "ColDelete", "Delete");
        ...
        private void ClearFareDetails()
        {
            ...
            numagentcomm.Value = 0.00m;
            numagentcharge.Value = 0.00m;
        ...
        private void AddDetail()
        {
            ...
            decimal agentcomm = numagentcomm.Value.ToDecimal();
            decimal agentcharge = numagentcharge.Value.ToDecimal();
            ...
            if (fromLocTypeId == Enums.LOCATION_TYPES.ADDRESS || toLocTypeId == Enums.LOCATION_TYPES.ADDRESS)
            {
                ...
                row.Cells[COLS_DETAILS.AGENTCOMM].Value = agentcomm;
                row.Cells[COLS_DETAILS.AGENTCHARGE].Value = agentcharge;
                ...
                else
                {
                    ...
                    foreach (RadListDataItem locFrom in lstFromLocation.Items)
                    {
                        ...
                        foreach (RadListDataItem locTo in lstToLocation.Items)
                        {
                            ...
                            detailRow.Cells[COLS_DETAILS.AGENTCOMM].Value = agentcomm;
                            detailRow.Cells[COLS_DETAILS.AGENTCHARGE].Value = agentcharge;
                        }
                    }
        ...
        private void ShowAgentDetails(bool show)
        {

            grdDetails.Columns[COLS_DETAILS.AGENTCOMM].IsVisible = show;
            grdDetails.Columns[COLS_DETAILS.AGENTCHARGE].IsVisible = show;
            numagentcharge.Visible = show;
            numagentcomm.Visible = show;
            lblAgentCharge.Visible = show;
            lblAgentComm.Visible = show;
        ...
        public override void Save()
        {
            try
            {
                ...
                if (FareId == 0)
                {
                    var query = General.GetObject<Fare>(c => (c.VehicleTypeId == VehicleTypeId) && (c.SubCompanyId == SubCompanyId) && ((CompanyId == null && c.CompanyId == null) || c.CompanyId == CompanyId)).DefaultIfEmpty().Id;
                    if (query > 0)
                    {
                        FareId = query;
                    }
                ...
                var existingRows = objMaster.Current.Fare_ChargesDetails.Select(c => c.Id).ToList();
                var updatedRows = grdDetails.Rows.Where(c => c.Cells[COLS_DETAILS.ID].Value.ToLong() > 0).Select(c => c.Cells[COLS_DETAILS.ID].Value.ToLong()).ToList();
                var deletedIds = existingRows.Where(c => updatedRows.Contains(c) == false).ToList();


                string recordsToRemove = string.Join(",", deletedIds.Select(c => c.ToStr()).ToArray<string>());
                if (recordsToRemove.ToStr().Trim().Length > 0)
                    recordsToRemove = "Delete from fare_chargesdetails where id in(" + recordsToRemove + ")";

                //var deletedRows = grdDetails.Rows.Where(c => c.Cells[COLS_DETAILS.ID].Value.ToLong() > 0).ToList();
                ...
                var newRows = grdDetails.Rows.Where(c => c.Cells[COLS_DETAILS.ID].Value.ToLong() == 0).ToList();
                ...
                foreach (var item in newRows)
                {
                    ...
                    //     if ((item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToInt() != 100))
                    originId = item.Cells[COLS_DETAILS.FROMLOCATIONID].Value.ToInt().ToStr();


                    //   if ((item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToInt() != 100))
                    destinationId = item.Cells[COLS_DETAILS.TOLOCATIONID].Value.ToInt().ToStr();


                    if (originId.ToStr().IsNumeric() && originId.ToInt() == 0)
                    {
                        originId = "NULL";
                    }
                    ...
                    if (destinationId.ToStr().IsNumeric() && destinationId.ToInt() == 0)
                    {
                        destinationId = "NULL";
                    }

                    if ((item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToInt() == 100) && (item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToInt() == 100))
                    {
                        insertQuery.AppendLine("INSERT INTO FARE_CHARGESDETAILS (FareId,OriginLocationTypeId,DestinationLocationTypeId,OriginId,DestinationId,FromZoneId,ToZoneId,FromAddress,ToAddress,Rate,CompanyRate, PeakTimeRate,OffPeakTimeRate)" +
                                           "Values(" + objMaster.Current.Id + "," + item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToIntorNull() + "," + item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToIntorNull() + ",NULL" + ",NULL" + "," + originId + "," + destinationId + ",'" + item.Cells[COLS_DETAILS.FROMLOCATION].Value.ToStr().Trim()
                                           + "','" + item.Cells[COLS_DETAILS.TOLOCATION].Value.ToStr().Trim() + "'," + item.Cells[COLS_DETAILS.FARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.COMPANYFARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.AGENTCOMM].Value.ToDecimal() + ", " + item.Cells[COLS_DETAILS.AGENTCHARGE].Value.ToDecimal() + ");");

                    }
                    else if ((item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToInt() == 100) && (item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToInt() != 100))
                    {
                        insertQuery.AppendLine("INSERT INTO FARE_CHARGESDETAILS (FareId,OriginLocationTypeId,DestinationLocationTypeId,OriginId,DestinationId,FromZoneId,ToZoneId,FromAddress,ToAddress,Rate,CompanyRate, PeakTimeRate,OffPeakTimeRate)" +
                                           "Values(" + objMaster.Current.Id + "," + item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToIntorNull() + "," + item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToIntorNull() + ",NULL" + "," + destinationId + "," + originId + ",NULL" + ",'" + item.Cells[COLS_DETAILS.FROMLOCATION].Value.ToStr().Trim()
                                           + "','" + item.Cells[COLS_DETAILS.TOLOCATION].Value.ToStr().Trim() + "'," + item.Cells[COLS_DETAILS.FARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.COMPANYFARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.AGENTCOMM].Value.ToDecimal() + ", " + item.Cells[COLS_DETAILS.AGENTCHARGE].Value.ToDecimal() + ");");
                    ...
                    {
                        insertQuery.AppendLine("INSERT INTO FARE_CHARGESDETAILS (FareId,OriginLocationTypeId,DestinationLocationTypeId,OriginId,DestinationId,FromZoneId,ToZoneId,FromAddress,ToAddress,Rate,CompanyRate, PeakTimeRate,OffPeakTimeRate)" +
                                           "Values(" + objMaster.Current.Id + "," + item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToIntorNull() + "," + item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToIntorNull() + "," + originId + ",NULL" + ",NULL" + "," + destinationId + ",'" + item.Cells[COLS_DETAILS.FROMLOCATION].Value.ToStr().Trim()
                                           + "','" + item.Cells[COLS_DETAILS.TOLOCATION].Value.ToStr().Trim() + "'," + item.Cells[COLS_DETAILS.FARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.COMPANYFARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.AGENTCOMM].Value.ToDecimal() + ", " + item.Cells[COLS_DETAILS.AGENTCHARGE].Value.ToDecimal() + ");");
                    ...
                    {
                        insertQuery.AppendLine("INSERT INTO FARE_CHARGESDETAILS (FareId,OriginLocationTypeId,DestinationLocationTypeId,OriginId,DestinationId,FromZoneId,ToZoneId,FromAddress,ToAddress,Rate,CompanyRate, PeakTimeRate,OffPeakTimeRate)" +
                                           "Values(" + objMaster.Current.Id + "," + item.Cells[COLS_DETAILS.FROMLOCTYPEID].Value.ToIntorNull() + "," + item.Cells[COLS_DETAILS.TOLOCTYPEID].Value.ToIntorNull() + "," + originId + "," + destinationId + ",NULL,NULL,'" + item.Cells[COLS_DETAILS.FROMLOCATION].Value.ToStr().Trim()
                                           + "','" + item.Cells[COLS_DETAILS.TOLOCATION].Value.ToStr().Trim() + "'," + item.Cells[COLS_DETAILS.FARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.COMPANYFARE].Value.ToDecimal() + "," + item.Cells[COLS_DETAILS.AGENTCOMM].Value.ToDecimal() + ", " + item.Cells[COLS_DETAILS.AGENTCHARGE].Value.ToDecimal() + ");");
                    }
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    if (insertQuery.ToStr().Trim().Length > 0)
                    {
                        db.ExecuteQuery<int>(insertQuery.ToStr().Trim());


                        newRows.ForEach(C => C.Cells[COLS_DETAILS.ID].Value = -1);
                    }
                    if (recordsToRemove.ToStr().Trim().Length > 0)
                    {
                        db.ExecuteQuery<int>(recordsToRemove.ToStr().Trim());
                    }
                }
                ...
                if (insertQuery.ToStr().Trim().Length > 0)
                {
                    ...
                    DisplayFareDetails(objMaster.Current);
                }
        ...
        private void DisplayFareDetails(Fare obj)
        {
            ...
            for (int i = 0; i < grdDetails.RowCount; i++)
            {
                ...
                grdDetails.Rows[i].Cells[COLS_DETAILS.AGENTCOMM].Value = obj.Fare_ChargesDetails[i].PeakTimeRate.ToDecimal();
                grdDetails.Rows[i].Cells[COLS_DETAILS.AGENTCHARGE].Value = obj.Fare_ChargesDetails[i].OffPeakTimeRate.ToDecimal();
            }
        ...
        public void FormatPlotWiseGrid(int FareId)
        {
            ...
            grdPlotWiseFare.MasterTemplate.Columns[0].IsPinned = true;
            grdPlotWiseFare.MasterTemplate.Columns[0].ReadOnly = true;

            if (grdPlotWiseFare.MasterTemplate.Columns[1].Name == "Plot")
            {

                grdPlotWiseFare.MasterTemplate.Columns[1].IsPinned = true;
                grdPlotWiseFare.MasterTemplate.Columns[1].ReadOnly = true;

                grdPlotWiseFare.MasterTemplate.Columns[0].Width = 70;
            }
        ...
        private void pg_mileage_Paint(object sender, PaintEventArgs e)
        {
