...
namespace Taxi_AppMain
{
    public partial class frmPayment : UI.SetupBase
    {
        ...
        private void SpinElement_TextChanging(object sender, TextChangingEventArgs e)
        {
            try
            {
                if (e.NewValue != "" && e.NewValue != "-" && e.NewValue != ".")
                {

                    if (e.NewValue.ToStr().Trim().Length > 0)
                    {

                        txtBalance.Value = (txtInvoiceTotal.Value.ToDecimal() - txtCreditNote.Value.ToDecimal()) - (e.NewValue.ToDecimal() + grdPayment.Rows.Sum(c => c.Cells[COL_PAYMENT.PAYMENT].Value.ToDecimal()));
                ...
                {
                    txtBalance.Value = (txtInvoiceTotal.Value.ToDecimal() - txtCreditNote.Value.ToDecimal()) - grdPayment.Rows.Sum(c => c.Cells[COL_PAYMENT.PAYMENT].Value.ToDecimal());
        ...
        decimal extraTotalValue = 0.00m;

        public frmPayment(int CompanyId, int InvoiceId,decimal extraTotal)
        {
            InitializeComponent();
            dtpPaymentDate.Value = DateTime.Now;
            FormatePaymenttGride();
        ...
        private void Display()
        {
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                  ...
                  string  TemplateName = db.UM_Form_Templates.FirstOrDefault(c => c.UM_Form.FormName == "frmInvoiceReport" && c.IsDefault == true).DefaultIfEmpty().TemplateName.ToStr();
                    ...
                    var item = (from a in db.Invoices.Where(c => (c.CompanyId == Id) && (InvoiceId == 0 || c.Id == InvoiceId))
                                ...
                                select new
                                {
                                    ...
                                    a.Gen_Company.VatOnlyOnAdminFees,
                                    a.CurrentBalance,
                                    a.Gen_Company.AdminFees,
                                    a.Gen_Company.DiscountPercentage,
                                    a.Gen_Company.AdminFeeType,
                    ...
                    if (TemplateName.ToStr().ToLower() != "template10" && TemplateName.ToStr().ToLower() != "template49")
                        extraTotalValue = 0.00m;


                    decimal Total = item.InvoiceTotal.ToDecimal()- extraTotalValue;
                    ...
                    if (item.HasVat.ToBool())
                    {
                        ...
                        if (item.VatOnlyOnAdminFees.ToBool())
                        {
                            decimal fees = (Total * adminFees) / 100;
                            fees = Math.Round(fees, 2);


                            if (item.AdminFeeType.ToStr() == "Amount")
                                fees = adminFees;

                            Total = Total + fees;
                            Total = Total + (fees * 20) / 100;
                    ...
                    else
                    {
                        ...
                        Total = (Total + (( (Total+ discountAmount) * adminFees) / 100));
                    ...
                    if (objCreditNote != null)
                    {
                        ...
                        txtCreditNote.Value = creditNoteTotal;
                    ...
                    if (query.Count() > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            row.Cells[COL_PAYMENT.PAYMENTDESCRIPTION].Value = dt.Rows[i]["PaymentDescription"];
        ...
        public struct COL_PAYMENT
        {
            ...
            public static string PAYMENTDESCRIPTION = "PAYMENTDESCRIPTION";
        ...
        private void AddPayment()
        {
            ...
            try
            {
                ...
                row.Cells[COL_PAYMENT.PAYMENTDESCRIPTION].Value = txtReason.Text.Trim();
        ...
        private void FormatePaymenttGride()
        {
            ...
            col.IsVisible = true;
            col.Name = COL_PAYMENT.PAYMENTDESCRIPTION;
            col.Width = 200;
            grdPayment.Columns.Add(col);
        ...
        public override void Save()
        {
            try
            {
               ...
               //     objMaster.Current.PaymentDescription= grdPayment.Rows.Sum(c => c.Cells[COL_PAYMENT.PAYMENTDESCRIPTION].Value.ToStr());
                    ...
                    List<invoice_Payment> listofDetail3 = (from r in grdPayment.Rows
                                                           ...
                                                           select new invoice_Payment
                                                           {
                                                               ...
                                                               AddOn = r.Cells[COL_PAYMENT.AddOn].Value.ToDateTimeorNull(),
                                                                  PaymentDescription= r.Cells[COL_PAYMENT.PAYMENTDESCRIPTION].Value.ToStr()
                                                           }).ToList();
