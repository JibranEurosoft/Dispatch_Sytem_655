...
namespace Taxi_AppMain
{
    public partial class frmAddMultipleCompanyInvoice : UI.SetupBase
    {
        ...
        public struct COLS_DETAILS
        {
            ...
            public static string NoteForInvoice = "NoteForInvoice";
        ...
        public void GetCompany()
        {
            try
            {
                ...
                colId.Width = 50;
                colId.AutoSizeMode = BestFitColumnMode.None;
                colId.HeaderText = "Acc Type";
                colId.Name = "AccountTypeId";
                colId.ReadOnly = true;
                grdCompany.Columns.Add(colId);
                ...
                colId.Width = 40;
                colId.AutoSizeMode = BestFitColumnMode.None;
                colId.HeaderText = "Notes";
                colId.Name = COLS_DETAILS.NoteForInvoice;
                colId.IsVisible = false;
                colId.ReadOnly = true;
                grdCompany.Columns.Add(colId);
        ...
        private void GetCompanyList()
        {
            ...
            try
            {
                ...
                var query = General.GetQueryable<Gen_Company>(null).Where(c => c.IsClosed == false &&
                         (c.Gen_Company_PaymentTypes.Count == 0 || c.Gen_Company_PaymentTypes.Any(a => a.PaymentTypeId == Enums.PAYMENT_TYPES.BANK_ACCOUNT || a.PaymentTypeId == 7))
                         && (subCompanyId == 0 || c.SubCompanyId == subCompanyId)
                         && (GroupId == 0 || c.GroupId == GroupId))

                        .OrderBy(c => c.CompanyName).Select(args => new { args.Id, args.CompanyName,args.SubCompanyId,args.NoteForInvoice ,args.AccountTypeId}).ToList();
                ...
                for (int i = 0; i < query.Count; i++)
                {
                    ...
                    grdCompany.Rows[i].Cells[COLS_DETAILS.NoteForInvoice].Value = query[i].NoteForInvoice.ToStr();

                    grdCompany.Rows[i].Cells["AccountTypeId"].Value = query[i].AccountTypeId;
                    ...
                    var list = (from b in listA
                        ...
                        if (chkSplitByDept.Checked)
                        {
                            ...
                            foreach (var item in deptList)
                            {
                                ...
                                objMaster.Current.BookingTypeId = detailRow.Cells["AccountTypeId"].Value.ToIntorNull();
                                ...
                                if (txtNotes.Text.Trim().Length > 0)
                                    ...
                                    objMaster.Current.Remarks = detailRow.Cells[COLS_DETAILS.NoteForInvoice].Value.ToStr();
                                ...
                                objMaster.Current.InvoiceTotal = listDept.Sum(callnotification => callnotification.Charges.ToDecimal() + callnotification.ParkingCharges.ToDecimal() + callnotification.WaitingCharges.ToDecimal() + callnotification.ExtraDropCharges.ToDecimal() + callnotification.ServiceCharges.ToDecimal());
                        ...
                        //    objMaster.Current.BookingTypeId = Enums.BOOKING_TYPES.LOCAL;
                                ...
                                objMaster.Current.InvoiceTotal = list.Sum(callnotification => callnotification.Charges.ToDecimal() + callnotification.ParkingCharges.ToDecimal() + callnotification.WaitingCharges.ToDecimal() + callnotification.ExtraDropCharges.ToDecimal()+ callnotification.ServiceCharges.ToDecimal());
                                ...
                                string[] skipProperties = { "Invoice", "Booking" };
                                IList<Invoice_Charge> savedList = objMaster.Current.Invoice_Charges;
                                List<Invoice_Charge> listofDetail = (from r in list
