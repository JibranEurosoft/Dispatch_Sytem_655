...
namespace Taxi_AppMain
{
    public partial class frmDespatchJob : Form
    {
        ...
        public bool ShowAllLoggedInDrivers = false;
             ...
             if (objJobCoord != null)
             {
                         ...
                         if (this.IsFojJob == false)
                         {
                             ...
                             using (TaxiDataContext db = new TaxiDataContext())
                             {
                                 ...
                                 var ListofPDAAvailDrvs = (from a in db.Fleet_DriverQueueLists.Where(c => c.Status == true  && c.DriverWorkStatusId == Enums.Driver_WORKINGSTATUS.AVAILABLE)
                                                           ...
                                                           select new
                                                           {
                                                               ...
                                                               Attributes=a.Fleet_Driver.AttributeValues
                            ...
                            if (objBooking.AttributeValues.ToStr().Trim().Length > 0)
                            {
                                ...
                                if (AppVars.listUserRights.Count(C => C.functionId == "ALLOW OVERRIDE DRIVER ATTRIBUTES ON DESPATCH JOB") > 0)
                                {
                                    ListofPDAAvailDrvs2 = ListofPDAAvailDrvs.Where(c => c.Attributes.ToStr().Contains(objBooking.AttributeValues.ToStr().Trim())).ToList();
                                }
                 ...
                 else
                 {
                     ...
                     using (TaxiDataContext db = new TaxiDataContext())
                     {
                         ...
                         var nearestDrivers = (from a in db.Fleet_DriverQueueLists
                                               ...
                                               join l in db.Fleet_Driver_Locations on a.DriverId equals l.DriverId
                                                       ...
                                                       DriverNo =d.DriverNo,
                                                        ...
                                                        VehicleTypeId=d.VehicleTypeId,
                                                         NoOfPassenger=v.NoofPassengers,
                                                           DrvLatitude=l.Latitude,
                                                            DrvLongitude=l.Longitude
                            ...
                            for (int i = 0; i < nearestDrivers.Count; i++)
                            {
                                ...
                                if (nearestDrivers[i].LocationName.ToStr().Trim().Length > 0)
                                {
                                    ...
                                    if (args != null)
                                    {
                                        nearestDrivers[i].Distance = new LatLng(Convert.ToDouble(nearestDrivers[i].DrvLatitude), Convert.ToDouble(nearestDrivers[i].DrvLongitude))
                                            .DistanceMiles(new LatLng(Convert.ToDouble(args.Latitude), Convert.ToDouble(args.Longtiude)));
        ...
        private void LoadAndSelectDriver()
        {
            try
            {
                if (ShowAllLoggedInDrivers)
                {
                    ComboFunctions.FillDriverNoLoggedInDespatchQueueCombo(ddl_Driver);

                    ddl_Driver.Items.Where(c => c.DataBoundItem != null && c.DataBoundItem.ToStr().Contains("DriverWorkStatusId = 2") || c.DataBoundItem.ToStr().Contains("DriverWorkStatusId = 4") || c.DataBoundItem.ToStr().Contains("DriverWorkStatusId = 5") || c.DataBoundItem.ToStr().Contains("DriverWorkStatusId = 6")).ToList().ForEach(c => c.ForeColor = Color.Blue);
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (AppVars.listUserRights.Count(c => c.functionId == "ALLOW DISPATCH JOB TO SINBIN DRIVER") > 0)
                            {

                                ComboFunctions.FillDriverNoQueueDespatchComboIncSinbin(ddl_Driver);
                                ddl_Driver.Items.Where(c => c.DataBoundItem != null && c.DataBoundItem.ToStr().Contains("DriverWorkStatusId = 7")).ToList().ForEach(c => c.ForeColor = Color.DimGray);
        ...
        private void Despatch()
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if(objBooking!=null && objBooking.GroupJobId!=null)
                    {

                        ENUtils.ShowMessage("You cannot dispatch this booking as its a part of shuttle");
                        return;
                    ...
                    try
                    {
                        if (ddl_Driver.SelectedItem != null && ddl_Driver.SelectedItem.DataBoundItem != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (ddl_Driver.SelectedItem.DataBoundItem.ToStr().Contains("DriverWorkStatusId = 7"))
                                {
                                    if (DialogResult.No == RadMessageBox.Show("This Driver is on SINBIN." + Environment.NewLine
            ...
            if (ObjDriver.VehicleTypeId != null)
            {
                ...
                if (objBooking.AttributeValues.ToStr().Trim().Length > 0)
                {
                    ...
                    if (matchCnt != totalAttr)
                    {
                        ...
                        if (AppVars.listUserRights.Count(C => C.functionId == "ALLOW OVERRIDE DRIVER ATTRIBUTES ON DESPATCH JOB") > 0)
                        {
                           if(DialogResult.No==  MessageBox.Show(("Driver : " + ObjDriver.DriverNo + " doesn't have attributes (" + unmatchedAttrValue + ")" + Environment.NewLine+"Do you want to override?"), "Warning", MessageBoxButtons.YesNo));
                            return;
                        }
                        else
                        {
                ...
                //    string specialReq = objBooking.SpecialRequirements.ToStr().Trim();
                    ...
                    else
                    {
                        ...
                        if (objBooking.EscortId != null)
                            companyName +=Environment.NewLine+ "ESCORT- " + objBooking.Gen_Escort.EscortName;
                        ...
                        try
                        {
                          ...
                          //  +
                            ...
                            if (ObjDriver.Id != 0 && objBooking.ZoneId!=null && FOJJob==string.Empty )
                            {
                                try
                                {
                                    if (AppVars.listUserRights.Count(c => c.functionId == "COMPULSARY JOB PARAMETER") > 0)
                                    {
                                        using (TaxiDataContext db = new TaxiDataContext())
                                        {
                                            db.CommandTimeout = 3;
                                            if (db.Fleet_Driver_Locations.Count(c => c.DriverId == ObjDriver.Id && c.ZoneId == objBooking.ZoneId)
                                               > 0)
                                            {
                            ...
                            if (!string.IsNullOrEmpty(tag.TagPropertyValue.ToStr()) && !string.IsNullOrEmpty(tag.TagPropertyValue2))
                            {
                                if (ObjDriver.Fleet_Driver_Images.Count > 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            string encrypt =Cryptography.Encrypt(objBooking.BookingNo.ToStr() + ":" + linkId+":"+AppVars.objPolicyConfiguration.ListenerIP.ToStr().Trim()+":"+objBooking.Id, "tcloudX@@!", true);
                ...
                try
                {
                  ...
                  //  if (this.JobStatusId == null)
                    ...
                    if ((!this.IsAutoDespatchActivity || !autoDespatch))
                    {
                        ...
                        if (enablePDA == false || ObjDriver.HasPDA==false || IsManualLogin)
                        {
                            ...
                            using (TaxiDataContext db = new TaxiDataContext())
                            {
                                ...
                                if (driverCurrent != 0)
                                {
                                    if ((enablePDA == false) || ( (ObjDriver.HasPDA.ToBool() == false || IsManualLogin) && enablePDA))
                                    {
                    ...
                    if (!this.IsAutoDespatchActivity)
                    {
                        ...
                        string displayName = ObjDriver.DisplayName!=null ?ObjDriver.DisplayName.ToStr(): ObjDriver.DriverNo.ToStr();
                        ...
                        {
                            General.SendMessageToPDA("request broadcast=" + RefreshTypes.REFRESH_DESPATCHJOB + ">>" + JobId + ">>" + ObjDriver.DriverNo.ToStr() + ">>" + Enums.BOOKINGSTATUS.PENDING + ">>" + (ObjDriver.HasPDA.ToBool() ? "pda" : "nonpda" ) + ">>" + displayName);
