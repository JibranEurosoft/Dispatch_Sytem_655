    ...
    public partial class frmMessageAllDrivers : Form
    {
        ...
        private void frmMessageAllDrivers_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    var list = (from q in db.Fleet_DriverQueueLists
                                join d in db.Fleet_Drivers on q.DriverId equals d.Id
                                where q.Status == true && (d.HasPDA != null && d.HasPDA == true) && d.IsActive == true
                                select new
                                {
                                    d.Id,
                                    d.DriverNo,
                    ...
                    //General.GetQueryable<Fleet_DriverQueueList>(c => c.Status == true && (c.Fleet_Driver.HasPDA != null && c.Fleet_Driver.HasPDA == true) && (c.Fleet_Driver.IsActive != null && c.Fleet_Driver.IsActive == true)).ToList();
                    ...
                    foreach (var query in list)
                    {
                        ...
                        RadListDataItem item = new RadListDataItem();
                        item.Font = new Font("Tahoma", 10, FontStyle.Bold);
                        item.Text = query.DriverNo + " - " + query.DriverName;
                        item.Value = query.Id;
                ...
                menuStrip1.Visible = AppVars.listUserRights.Count(c => c.functionId == "SCHEDULE MESSAGING") > 0;
            }
            catch (Exception ex)
            {
                ENUtils.ShowMessage(ex.Message);
        ...
        private void SendMessage()
        {
            try
            {
                ...
                if(chkAllDrivers.Checked==false && radListControl1.Items.Count(c=>c.Selected==true)==0)
                {
        ...
        private void SendPDAMessage(string msg,RadListDataItemCollection items)
        {
            ...
            try
            {
                ...
                if(chkAllDrivers.Checked==false)
                {
                    drvIds = string.Join(",", items.Where(c=>c.Selected).Select(c => c.Value.ToStr()).ToArray<string>());
        ...
        private void scheduleMessagesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            frmScheduleMessage frm = new frmScheduleMessage();
            frm.ShowDialog();
        }
        ...
        private void chkAllDrivers_CheckedChanged(object sender, EventArgs e)
        {
            if (chkAllDrivers.Checked)
