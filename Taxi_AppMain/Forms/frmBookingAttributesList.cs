...
namespace Taxi_AppMain
{
    public partial class frmBookingAttributesList : Form
    {
        ...
        private void FormatBookingAttributesListGrid()
        {
            ...
            grdAttributesList.AllowUserToAddRows = false;
        ...
        private void LoadBookingAttributesListGrid()
        {
            ...
            try
            {
                var list = General.GetQueryable<Gen_Attribute>(c=>c.AttributeCategoryId==null).OrderBy(c => c.Name).ToList();
        ...
        private void SaveAndClose()
        {
            ...
            try
            {
                ...
                string new_input_values = string.Join(",", grdAttributesList.Rows.Cast<DataGridViewRow>().Where(r => r.Cells[Col_BookingAttributesList.Default].Value.ToBool() == true).Select(c => c.Cells[Col_BookingAttributesList.ShortName].Value.ToStr()).ToArray<string>());


                if (input_values.ToStr().Trim().Length == 0 && new_input_values.ToStr().Trim().Length == 0)
                {
                    MessageBox.Show("Please select any attribute");
                    return;
                }
