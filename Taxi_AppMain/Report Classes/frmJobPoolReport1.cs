...
namespace Taxi_AppMain
{
    public partial class frmJobPoolReport1 : UI.SetupBase
    {
        ...
        void frmJobPoolReport1_Load(object sender, EventArgs e)
        {
            ...
            dtp_AcceptedFrom.Value = DateTime.Now.AddDays(-7).ToDate();
            dtp_AcceptedTo.Value = DateTime.Now.AddDays(1).Date+  new TimeSpan(23,59,59);

            dtp_TransferredFrom.Value = DateTime.Now.AddDays(-7).ToDate();
            dtp_TranferredTo.Value = DateTime.Now.AddDays(1).Date + new TimeSpan(23, 59, 59);
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    var list = (from p in db.Bookings
                               ...
                               join s in db.BookingStatus on p.BookingStatusId equals s.Id
                                  ...
                                  where (p.BookingStatusId==21 ||p.BookingStatusId==25  || p.BookingStatusId==27) && ( (p.PickupDateTime>=dtp_TransferredFrom.Value && p.PickupDateTime <= dtp_TranferredTo.Value))
                                  ...
                                  select new
                                  {
                                   ...
                                   //  Driver=p.CompanyCreditCardDetails,
                                      AcceptedSubCompanies = p.CompanyCreditCardDetails,
                                      DriverSignCall = p.NotesString
            ...
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    var list = (from p in db.Bookings
                               ...
                               where p.BookingTypeId == 100 &&  ( (p.PickupDateTime >= dtp_AcceptedFrom.Value && p.PickupDateTime <= dtp_AcceptedTo.Value))
                                ...
                                select new
                                {
                                    ...
                                    Driver=p.DriverId!=null? p.Fleet_Driver.DriverNo:"",
                                    // AcceptedSubCompanies=
                                    OriginSubCompany = p.CompanyCreditCardDetails
