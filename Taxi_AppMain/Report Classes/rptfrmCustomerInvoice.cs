...
namespace Taxi_AppMain
{
 ...
 //       }
        ...
        public void GenerateReport()
        {
            ...
            var findBookingId = this.DataSource;
          //  string BookingFee = "0";
            int bid = 0;
            foreach (var item in findBookingId)
            {
                 bid = (int)item.BookingId;
                
            }
            TaxiDataContext db = new TaxiDataContext();
            
            var bookingFee = db.Bookings
                    .Where(b => b.Id == bid)
                    .Select(b => b.ServiceCharges)
                    .FirstOrDefault();


            string bookingFeeValue = bookingFee != null ? Convert.ToInt32(bookingFee).ToString() : "0";
        ...
        string footer = string.Empty;
            ...
            decimal invoiceGrandTotal = this.DataSource.FirstOrDefault().DefaultIfEmpty().InvoiceTotal.ToDecimal() + this.DataSource.FirstOrDefault().DefaultIfEmpty().AdminFees.ToDecimal();
            string grandTotal =  invoiceGrandTotal.ToStr();
            //grandTotal = grandTotal.Substring(1);
            param[3] = new Microsoft.Reporting.WinForms.ReportParameter("Report_Parameter_InvoiceTotal", grandTotal);
            ...
            param[10] = new Microsoft.Reporting.WinForms.ReportParameter("Report_Parameter_BookingFee", bookingFeeValue);
