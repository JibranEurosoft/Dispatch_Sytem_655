...
namespace Taxi_AppMain
{
    public partial class frmAgentCommissionReport : UI.SetupBase
    {
        ...
        public frmAgentCommissionReport()
        {
            ...
            ComboFunctions.FillZonesCombo(ddlZones);
        ...
        void frmDriverReport_Load(object sender, EventArgs e)
        {
            ...
            ComboFunctions.FillLocTypeCombo(ddlLocType);
            ddlLocType.Text = "Plot";
            ...
            ddlLocType.SelectedIndexChanged += DdlLocType_SelectedIndexChanged;
            ...
            if (this.TemplatePath.ToStr().ToLower() == "template3" || this.TemplatePath.ToStr().ToLower() == "template4")
            {
                ...
                if (this.TemplatePath.ToStr().ToLower() == "template4")
                {
                   ...
                   // lblAgent.Visible = false;
                    ...
                    pnlPaymentTypes2.Visible = false;
        ...
        private void DdlLocType_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            int locTypeId = ddlLocType.SelectedValue.ToInt();

            if (ddlLocType.Text == "Plot")
            {
                ComboFunctions.FillZonesCombo(ddlZones);
            }
            else
            {
                ComboFunctions.FillLocationsCombo(ddlZones, c => c.LocationTypeId == locTypeId);
            }
        }
        ...
        public override void Print()
        {
            ...
            if (ddlLocType.Text == "Plot")
            {
                zoneId = ddlZones.SelectedValue.ToInt();
            }
            else
            {
                postcode = ddlZones.SelectedValue.ToStr();
            }
            ...
            if (tillDate != null)
                tillDate = dtpTillDate.Value.ToDate() + TimeSpan.Parse("23:59:59");
            ...
            if (opCreditCardPaid.IsChecked)
            {
                paymenttype = Enums.PAYMENT_TYPES.CREDIT_CARD_PAID;
            }

            int paymenttype2 = 0;
                
            if (pnlPaymentTypes2.Visible==true)
            { 
                //Option button Run
                this.DataSource = GetDataSource(zoneId, postcode, companyId, departmentId, fromDate, tillDate, BookedBy, GroupId, paymenttype);
            }
            else
            {
                //CheckBox button Run
                this.DataSource = GetDataSource2(zoneId, postcode, companyId, departmentId, fromDate, tillDate, BookedBy, GroupId, paymenttype2);
            }
            GenerateReport();
        }

        List<Vu_AgentCommissionReport> _DataSource;

        public List<Vu_AgentCommissionReport> DataSource
        {
        ...
        private void ReInitializeReportViewer()
        {
            ...
            reportDataSource3.Name = "Vu_AgentCommissionReport";
            reportDataSource3.Value = this.Vu_AgentCommissionReportBindingSource;
            ...
            {
                //Taxi_AppMain.ReportDesigns.Template4_rptAgentCommissionStatementWithSummary_hideCharges.rdlc
                
                if (chkShowFares.Checked)
                {
                    this.reportViewer1.LocalReport.ReportEmbeddedResource = "Taxi_AppMain.ReportDesigns.Template4_rptAgentCommissionStatementWithSummary_hideFairs.rdlc";
                }
                else
                {
                    this.reportViewer1.LocalReport.ReportEmbeddedResource = "Taxi_AppMain.ReportDesigns.Template4_rptAgentCommissionStatementWithSummary_hideCharges.rdlc";
                }
            ...
            else
            {
                ...
                if (chkShowFares.Checked)
                {
                    this.reportViewer1.LocalReport.ReportEmbeddedResource = "Taxi_AppMain.ReportDesigns.Template4_rptAgentCommissionStatementWithSummaryFairs.rdlc";
                }
                else
                {
        ...
        public void GenerateReport()
        {
            ...
            if (objTemplate.TemplateName.ToStr() == "Template4")
            {
                ...
                if (opCreditCardPaid.IsChecked)
                {
                    reportHeading = AppVars.objSubCompany.CompanyName + " Agent Credit Card (Paid) Commission Report";
                }
            ...
            if (objTemplate != null)
            {
                if (chkShowCharges.Checked)
                {
                    ...
                    if (objTemplate.TemplateName.ToStr() == "Template4")
                    {
                        ...
                        //new report name = Template_rptAgentCommissionStatementWithSummary_hideCharges
                        //old report name=  _rptAgentCommissionStatementWithChargesWithSummary.rdlc
                        ...
                        {
                            //this.reportViewer1.LocalReport.ReportEmbeddedResource = "Taxi_AppMain.ReportDesigns.Template4_rptAgentCommissionStatementWithSummary_hideFairs.rdlc";
                            this.reportViewer1.LocalReport.ReportEmbeddedResource = reportPath + objTemplate.TemplateName.ToStr().Trim() + "_rptAgentCommissionStatementWithSummary_hideFairs.rdlc";
                        }
                        else
                        {
                            this.reportViewer1.LocalReport.ReportEmbeddedResource = reportPath + objTemplate.TemplateName.ToStr().Trim() + "_rptAgentCommissionStatementWithSummary_hideCharges.rdlc";
                        }
                ...
                else
                {
                    if (objTemplate.TemplateName.ToStr() == "Template4")
                    {
                        ...
                        {
                            //this.reportViewer1.LocalReport.ReportEmbeddedResource = "Taxi_AppMain.ReportDesigns.Template4_rptAgentCommissionStatementWithSummary_hideFairs.rdlc";
                            this.reportViewer1.LocalReport.ReportEmbeddedResource = reportPath + objTemplate.TemplateName.ToStr().Trim() + "_rptAgentCommissionStatementWithSummaryFairs.rdlc";
            ...
            this.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("Taxi_Model_Vu_BookingBase", this.DataSource));
            this.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("Taxi_AppMain_Classes_ClsLogo", objLogo));

          

            //  this.Vu_AgentCommissionReportBindingSource.DataSource = this.DataSource;
        ...
        private List<Vu_AgentCommissionReport> listofTempDataSource = new List<Vu_AgentCommissionReport>();
        ...
        private List<Vu_AgentCommissionReport> GetDataSource( int ZoneId, string postCode, int companyId, int departmentId, DateTime? fromDate, DateTime? tillDate, string BookedBy, int CompanyGroupId,int paymenttype)
        {

            //if (pnlPaymentTypes2.Visible == false)
            ...
            var list= General.GetQueryable<Vu_AgentCommissionReport>(c => (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED)
               
                && (companyId == 0 || c.CompanyId == companyId)
                && (ZoneId == 0 ||  (c.ZoneId == ZoneId || c.DropOffZoneId==ZoneId )) 
                                                && (departmentId == 0 || c.DepartmentId == departmentId)
                                                && (CompanyGroupId == 0 || c.CompanyGroupId == CompanyGroupId)
                                             && (BookedBy == string.Empty || c.BookedBy == BookedBy)
                                              && (postCode == string.Empty || c.fromPostCode == postCode)
                                               && (postCode == string.Empty || c.ToPostCode == postCode)
                                             &&
                                             (
                                             (paymenttype == 0 && (c.PaymentTypeId == Enums.PAYMENT_TYPES.CASH || c.PaymentTypeId == Enums.PAYMENT_TYPES.CREDIT_CARD || c.PaymentTypeId == Enums.PAYMENT_TYPES.CREDIT_CARD_PAID))
                                             || (paymenttype == -1 && (c.PaymentTypeId != 0))
                                             || (c.PaymentTypeId == paymenttype)
                                             ))
            ...
            System.Collections.Generic.List<Vu_AgentCommissionReport> listofData = new List<Vu_AgentCommissionReport>();
            Vu_AgentCommissionReport obj = null;
            foreach (var item in list)
            {
                obj = new Vu_AgentCommissionReport();
                ...
                obj.AgentCharge = item.AgentCharge;
                obj.FareRate = item.FareRate.ToDecimal();
                obj.ExtraDropCharges= item.BookingStatusId == Enums.BOOKINGSTATUS.NOPICKUP ? 0.00m : item.ExtraDropCharges.ToDecimal();
                obj.MeetAndGreetCharges = item.BookingStatusId == Enums.BOOKINGSTATUS.NOPICKUP ? 0.00m : item.MeetAndGreetCharges.ToDecimal();
                listofData.Add(obj);
        ...
        private List<Vu_AgentCommissionReport> GetDataSource2(int ZoneId, string postCode, int companyId, int departmentId, DateTime? fromDate, DateTime? tillDate, string BookedBy, int CompanyGroupId, int paymenttype)
        {
            //
            ...
            if (chkBoth.Checked)
            {                
                listofpaymenttypes.Add(1);
                listofpaymenttypes.Add(2);
                listofpaymenttypes.Add(3);
                listofpaymenttypes.Add(6);
            ...
            var list = General.GetQueryable<Vu_AgentCommissionReport>(c => (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED)
                 ...
                 && (ZoneId == 0 || (c.ZoneId == ZoneId || c.DropOffZoneId == ZoneId))
                                              ...
                                              //|| (c.PaymentTypeId == paymenttype)
                                              //)
                                              (listofpaymenttypes.Contains(c.PaymentTypeId))
            ...
            foreach (var item in list)
            {
                ...
                obj.DriverFullName = item.DriverFullName;
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<Vu_AgentCommissionReport> listRpt = GetDataSource(zoneId, postcode, companyId, departmentId, fromDate, tillDate, BookedBy, GroupId, paymenttype);
                this.DataSource = listRpt;//GetDataSource(zoneId, postcode, companyId, departmentId, fromDate, tillDate, BookedBy, GroupId, paymenttype);
                GenerateReport();
                ...
                radGridView2.Columns.Add(new GridViewTextBoxColumn("PickupDateTime", "PickupDateTime"));
                radGridView2.Columns.Add(new GridViewTextBoxColumn("FromAddress", "FromAddress"));
                radGridView2.Columns.Add(new GridViewTextBoxColumn("NoOfVias", "NoOfVias"));
                ...
                radGridView2.Columns.Add(new GridViewTextBoxColumn("AgentCommission", "AgentCommission"));
                radGridView2.Columns.Add(new GridViewTextBoxColumn("AgentCharge", "AgentCharge"));
                radGridView2.Columns.Add(new GridViewTextBoxColumn("Fare", "Fare"));
                radGridView2.Columns.Add(new GridViewTextBoxColumn("Total", "Total"));


                radGridView2.RowCount = listRpt.Count;
                for (int i = 0; i < radGridView2.RowCount; i++)
                {

                    radGridView2.Rows[i].Cells["PickupDateTime"].Value = string.Format(" {0:dd/MM/yyyy HH:mm} ", listRpt[i].PickupDateTime) + "  ";
                    radGridView2.Rows[i].Cells["FromAddress"].Value = listRpt[i].FromAddress.ToString() + "  ";
                    radGridView2.Rows[i].Cells["NoOfVias"].Value = listRpt[i].NoOfVias.ToInt();
                    radGridView2.Rows[i].Cells["ToAddress"].Value = listRpt[i].ToAddress.ToString() + "  ";
                    radGridView2.Rows[i].Cells["CustomerName"].Value = listRpt[i].CustomerName.ToString() + "  ";
                    radGridView2.Rows[i].Cells["AgentCommission"].Value = listRpt[i].AgentCommission.ToDecimal();
                    radGridView2.Rows[i].Cells["AgentCharge"].Value = listRpt[i].AgentCharge.ToDecimal();

                    radGridView2.Rows[i].Cells["Fare"].Value = listRpt[i].FareRate.ToDecimal() + listRpt[i].WaitingCharges.ToDecimal() + listRpt[i].ExtraDropCharges.ToDecimal();

                    radGridView2.Rows[i].Cells["Total"].Value = listRpt[i].AgentCommission.ToDecimal() + listRpt[i].AgentCharge.ToDecimal() + listRpt[i].FareRate.ToDecimal() +
                                                                listRpt[i].WaitingCharges.ToDecimal() + listRpt[i].ExtraDropCharges.ToDecimal();
                }


                radGridView2.Columns["PickupDateTime"].HeaderText = "Pickup D/T";
                radGridView2.Columns["FromAddress"].HeaderText = "Pickup";
                radGridView2.Columns["NoOfVias"].HeaderText = "No Of Vias";
                radGridView2.Columns["ToAddress"].HeaderText = "Destination";
                radGridView2.Columns["CustomerName"].HeaderText = "Passenger";
                radGridView2.Columns["AgentCommission"].HeaderText = "Agent Fee";
                radGridView2.Columns["AgentCharge"].HeaderText = "Agent Charge";
                radGridView2.Columns["Fare"].HeaderText = "Fare";
                radGridView2.Columns["Total"].HeaderText = "Total";
                ...
                if (radGridView2.Rows.Count > 0)
                {
                    ...
                    sfd.FileName = "Agent Commission Report - To Pay.csv";
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (!fileError)
                        {
                            ...
                            for (int i = radGridView2.Columns["PickupDateTime"].Index; i < radGridView2.Columns.Count; i++)
                            {
                            ...
                            {
                                for (int j = radGridView2.Columns["PickupDateTime"].Index; j < columnCount; j++)
                                {
        ...
        private void chkBoth_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                ...
                chkCreditCardPaid.Checked = false;
                chkAccount.Checked = false;
        ...
        private void chkAccount_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {                
            ...
            {
                if (chkAccount.Checked == false && chkCash.Checked == false && chkCreditCard.Checked == false && chkCreditCardPaid.Checked == false && chkAccount.Checked == false)
                { 
                    chkBoth.Checked = true;                
                }
                else
        ...
        private void chkCash_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
        ...
        private void chkCreditCard_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
        ...
        private void chkCreditCardPaid_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                chkCreditCardPaid.Checked = true;
                chkBoth.Checked = false;
            }
            else
            {
