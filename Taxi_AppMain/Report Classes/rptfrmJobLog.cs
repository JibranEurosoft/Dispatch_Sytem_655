                ...
                if (driverId > 0)
                {
                    using (TaxiDataContext db = new TaxiDataContext())
                    {
                        ...
                        if (vehicleDAta != null)
                        {
                            ...
                            try
                            {
                                jobId = this.DataSource.FirstOrDefault().Id;
                                fleetMasterId = db.Bookings.Where(c => c.Id == jobId).Select(c => c.FleetMasterId).FirstOrDefault().ToInt();

                                if (fleetMasterId > 0)
                                {
                                    vehicleDetails = db.Fleet_Masters.Where(c => c.Id == fleetMasterId).Select(c => "Vehicle Details : " + c.VehicleMake + " - " + c.VehicleModel + " - " + c.VehicleNo).FirstOrDefault();
