...
{
    public partial class rptJobRouthPathGoogleOtherOffice: Form
    {
        ...
        public rptJobRouthPathGoogleOtherOffice()
        {
            InitializeComponent();
        }
        ...
        public rptJobRouthPathGoogleOtherOffice(Booking obj, bool TrackDriver)
        {
            InitializeComponent();
            this.ObjBooking = obj;
        ...
        public rptJobRouthPathGoogleOtherOffice(Booking obj, bool TrackDriver,int driverId)
        {
            InitializeComponent();
            this.ObjBooking = obj;
            IsTrackDriver = TrackDriver;
        ...
        long JobId;
        JobPoolAPICaller.TrackDriverDetail objTrackDriverDetails;
        public rptJobRouthPathGoogleOtherOffice(long jobId,JobPoolAPICaller.TrackDriverDetail objDetails)
        {
            InitializeComponent();
            this.JobId = jobId;
            objTrackDriverDetails = objDetails;
        ...
        private void OnDisplayDriverLocation()
        {
            try
            {
                ...
                string result = JobPoolAPIProxy.TrackDriver(this.JobId.ToStr(),objTrackDriverDetails.poolBookingId,objTrackDriverDetails.acceptorId,objTrackDriverDetails.acceptorJobId);
                RequestResponse<JobPoolAPICaller.TrackDriverDetail> objX = Newtonsoft.Json.JsonConvert.DeserializeObject<RequestResponse<TrackDriverDetail>>(result);
                objTrackDriverDetails = objX.Data;
                //   objTrackDriverDetails= Newtonsoft.Json.JsonConvert.DeserializeObject<JobPoolAPICaller.TrackDriverDetail>(result);

                var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<JobPoolAPICaller.TrackDriver>(objTrackDriverDetails.trackDriverDetail.TrimStart('[').TrimEnd(']'));
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    if (objTrackDriverDetails != null)
                    {
                       // string locName = obj.LocationName;

                        if (markerDrv == null)
                        {
                            markerDrv = new GMapMarkerCustom(new PointLatLng(Convert.ToDouble(obj.DriverLatitude), Convert.ToDouble(obj.DriverLongitude)), null);
                            ...
                            markerDrv.Position = new PointLatLng(Convert.ToDouble(obj.DriverLatitude), Convert.ToDouble(obj.DriverLongitude));
                        ...
                        //{
                            ...
                            if (true || (string.IsNullOrEmpty(markerDrv.PDALocationName)))
                            {

                                //if (obj.Speed > 0 || (string.IsNullOrEmpty(markerDrv.PDALocationName)))
                                //{

                                //if (AppVars.objPolicyConfiguration.EnablePOI.ToBool() && Convert.ToDouble(obj.DriverLatitude)>0)
                                //{

                                //    locName = db.PostCodesNearLatLong(obj.DriverLatitude, obj.DriverLongitude).FirstOrDefault().DefaultIfEmpty().Street.ToStr();
                                    ...
                                    //{
                                        ...
                                        if (oddCnt % 2 == 1)
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                GetDistance.Coords obj1 = new GetDistance.Coords { Latitude =Convert.ToDouble(obj.DriverLatitude), Longitude =Convert.ToDouble(obj.DriverLongitude) };
                      ...
                      //  }
                        ...
                        markerDrv.ToolTipText = DriverNo + Environment.NewLine + "Badge No:" + obj.BadgeNumber.ToStr().ToUpper();
                        //needtouncomment
                            //marketDrv.ToolTipText+= Environment.NewLine + "Speed: " + Math.Round(Convert.ToDouble(obj.Speed), 0) + "mph";
                      ...
                      //needtouncomment
                       ...
                       // var queue = db.Fleet_DriverQueueLists.Where(c =>c.DriverId==this._DriverId && c.Status == true).Select(args=>new {args.DriverWorkStatusId }).FirstOrDefault();
                        ...
                        if (obj != null)
                        {
                            ...
                            int StatusId = obj.BookingStatusId.ToInt();

                           
                         
                            if (StatusId == Enums.BOOKINGSTATUS.DISPATCHED || StatusId==100)
                            { 
                                imagePath += "diamond_Available.png";  //+ item.workstatus.Strip(' ') + ".png"
                                status = "Available";
                            }

                            else if (StatusId == Enums.BOOKINGSTATUS.ONROUTE)
                            {
