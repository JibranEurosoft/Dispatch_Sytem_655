    ...
    public class JATEmail
    {
        ...
        public static void CustomerCancelationEmail(Booking obj)
        {
            ...
            try
            {
                ...
                if(obj.IsQuotation.ToBool())
                    Subject = "QUOTATION CANCELLATION - " + string.Format("{0:dd MMMM yyyy}", obj.PickupDateTime) + ", TIME " + string.Format("{0:HH.mm}", obj.PickupDateTime) + " - " + obj.BookingNo.ToStr();
                else
                Subject = "BOOKING CANCELLATION - " + string.Format("{0:dd MMMM yyyy}", obj.PickupDateTime) + ", TIME " + string.Format("{0:HH.mm}", obj.PickupDateTime) + " - " + obj.BookingNo.ToStr();
                ...
                if(obj.IsQuotation.ToBool())
                {
                    sb.Append("Your Quotation has been cancelled at " + string.Format("{0:ddd dd MMM yyyy HH:mm}", obj.PickupDateTime));
                ...
                string faresection = "£" + string.Format("{0:f2}", fares) + " (" + obj.Gen_PaymentType.DefaultIfEmpty().PaymentType + ")";


                if (AppVars.listUserRights.Count(c => c.functionId == "AUTO SEND CONFIRMATION EMAIL") > 0)
                {

                    faresection = obj.Gen_PaymentType.DefaultIfEmpty().PaymentType;
                }
                ...
                if (footer.ToStr().Trim().Length > 0)
                    sb.Append("<tr><td colspan='2' style='border-bottom: #d4e0ee 1px solid;font-family:Arial;font-size:12px;font-weight:normal;'>" + footer + "</td></tr>");
        ...
        public static void SendEmail()
        {
            ...
            try
            {
                ...
                {
                    ClsEASendEmail em = new ClsEASendEmail(From, To, Subject, Body, smtpHost,ccEmail);
                    em.Send("", userName, pwd);
